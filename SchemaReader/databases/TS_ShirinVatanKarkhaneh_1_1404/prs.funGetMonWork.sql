...
-- ==============================================
Create FUNCTION prs.funGetMonWork
(
	@BasePay int,
	@MonthCode Tinyint ,
...
Declare @MonthlyFunction as integer
Declare @BasePay2 as integer
Declare @MonthCode2 as integer

select top 1 @MonthlyFunction =MonthlyFunction  from prs.tblFunctionsDtl
...
and ProcessID in (320,325)
	...
	if @BasePay2=@BasePay
		set @Ret=@MonthlyFunction
	else
	BEGIN
		if  @MonthlyFunction<>0
			RETURN 0
		set @Ret=0
		IF @MonthCode = 12 
		BEGIN
			...
			IF @Ret>0
			BEGIN
				select @MonthlyFunction =MonthlyFunction  from prs.tblFunctionsDtl
				where PersonnelID=@PersonnelID and MonthCode=@MonthCode
