...
-- Last Modified : 
-- Last Modifier : 
-- Description	 : گزارش لیست برگه های خدمات - خلاصه
-- ==============================================
Create PROCEDURE [acc].[RptAcc_ServicesList_Summary]
	@ProcessNo			Int = 1,
	@SerialNoFr			Int = Null,
	@SerialNoTo			Int = Null,
	@VchNoFr			Int = Null,
...
Begin --============== S T A R T  C O D E ===================================================
	...
	SET @SelectedService2  = pub.funSplitString(@ExtraParams, '@', 1);	
	SET @SelectedService3  = pub.funSplitString(@ExtraParams, '@', 2);
	SET @SelectedService4  = pub.funSplitString(@ExtraParams, '@', 3);
	SET @SelectedVisitor1  = pub.funSplitString(@ExtraParams, '@', 4);
	SET @SelectedVisitor2  = pub.funSplitString(@ExtraParams, '@', 5);
	SET @SelectedVisitor3  = pub.funSplitString(@ExtraParams, '@', 6);
	SET @SelectedVisitor4  = pub.funSplitString(@ExtraParams, '@', 7);
	SET @TaxToll		   = pub.funSplitString(@ExtraParams, '@', 8);		
	...
	-- SELECT Clause ----------------------------------------
	SET @StrSelect = '
	SELECT	H.SerialNo, H.DocDate, H.AcntCode, H.ServiceDiscount,H.TotalLineDiscount,H.Price,H.Amount, H.TaxOverWorthCost,H.TollOverWorthCost,
			pub.GetCodeName(H.AcntCode, ' + @LangID + ') AcntName,
			Sum(D.ServiceQuantity * D.ServiceAmount) AS SumPrice,EconomicalCode,Address1,Address2,NationalIDNumber,NationalIdentity
	...
	WHERE ' + @StrWhere + '
	GROUP BY H.SerialNo, H.DocDate, H.AcntCode, H.ServiceDiscount, H.TaxOverWorthCost,H.TollOverWorthCost,H.TotalLineDiscount,H.Price,H.Amount,EconomicalCode,Address1,Address2,NationalIDNumber,NationalIdentity
	ORDER BY ' + @SortFields
	------------------------------------------------------------
