...
-- Description: آمار (یا وضعیت) درخواست یک کالا یا یک درخواست کننده
-- ==============================================
CREATE PROCEDURE [cmr].[RptCMR_Statistics]
	@ProcessNo		TinyInt = 1,
...
Begin --============== S T A R T  C O D E =====================================
	...
	Set @StrSelect = '
	SELECT	T.' + @CodeField + ', ' + @NameField + ', Sum(T.ConfirmQuantity) AS RequestedQty, 
	...
	(
		...
		WHERE   ' + @StrWhere + '
		GROUP BY D.ProcessID, D.ProcessNo, D.FiscalYear, D.SerialNo, D.DocRowNo, D.DocDate, D.' + @CodeField + ', D.ConfirmQuantity
	) T 
