...
-- ==============================================
CREATE PROCEDURE [trs].[RptTrs_PayableDocs_Chart]
	@ProcessNo			int = 1,
	...
	@CreditCode3		Int = 0,
	@CreditCode4		Int = 0,
	@RepOptions			VarChar(10) = '0',
	@RepInfo			NVarChar(100) = Null
WITH ENCRYPTION
As
DECLARE @StrSelect		NVarChar(2000);
DECLARE @StrSelectSub	NVarChar(2000);
DECLARE @StrSelectPrm 	NVarChar(2000);
...
Begin   
	...
	SET @DailyChq	= Substring(@RepOptions, 1, 1);
   ...
   (
	...
	Set @StrSelect = @StrSelect + '
		WHERE (D.ProcessID IN (2,25))
			and (D.ProcessNo = ' + LTRIM(STR(@ProcessNo)) + ') 
			and (D.PayTypeID IN (' + @paytyp + '))'
