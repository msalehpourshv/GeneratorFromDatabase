...
-- =============================================
Create PROCEDURE crm.sp_AutoPreSaleHdrForRozCRM
@ProcessNo as int,
@DocDate as CHAR(10),
...
if (select count(*) from inv.tblPreSaleHdr where TransferSerialNo=@TransferSerialNo)=0
begin	
	...
	BEGIN
			...
			raiserror (@StrErrorMessage, 16, 1)
	END	
	IF (SELECT Count(*) FROM sal.tblTransporters WHERE TransporterID=@TransporterID )=0 and @TransporterID<>''
	BEGIN
			Set @StrErrorMessage = N' کد نوع ارسال توسط  '+@TransporterID+' نامعتبر است '
			raiserror (@StrErrorMessage, 16, 1)
	END	
	...
	(ProcessID, ProcessNo, FiscalYear, SerialNo, DocStep, DocDate,	StoreID, AcntCode,AcntName,
	 DiscountPercent, Discount, Discount2, DocDesc,SaleTypeID,
	 TaxOverWorthCost, TollOverWorthCost,DocDate2,
	 TransportationIncomeAcntCode,TransportationIncome,TransferSerialNo,VisitorAcntCode,TransporterID,TransportationCostAcntCode,TransportationCost,SessionNo)
	
	SELECT 240, @ProcessNo, @FiscalYear, @maxSerialNo, 1, @DocDate, @StoreID, @AcntCode, pub.GetCodeName(@AcntCode,1),
...
End
	...
	SELECT   SerialNo, AcntCode,StoreID,SaleTypeID,1 IsExist from inv.tblPreSaleHdr where TransferSerialNo=@TransferSerialNo
