...
-- Description   : 
-- =============================================
CREATE PROCEDURE inv.sp_api_AppService_GetSalesHdr
@Skip AS  nvarchar(50),
...
BEGIN TRY
	...
	SET @strQuery='
					SELECT SerialNo ,FiscalYear,ProcessNo ,cast (ProcessID as tinyint) as ProcessID,H.AcntCode ,f.AcntName ,VisitorAcntCode ,f.AcntName AS  VisitorAcntName ,VisitorAcntCode,Amount ,DocDate ,DocDesc ,Price ,Discount ,
					       case when isnull(f.Mobile,'''') =''  then isnull(f.SMSMobile,'''')
								when isnull(f.Mobile,'''') <> '' then  isnull(f.Mobile ,'''')
							end Mobile,
					...
					WHERE ProcessNo= '+@ProcessNo + @AcntCustomerCode
	IF(@VisitorID<>'')
