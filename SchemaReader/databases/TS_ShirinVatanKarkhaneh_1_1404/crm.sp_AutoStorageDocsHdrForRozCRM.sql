...
-- Description   : 
-- =============================================
Create PROCEDURE crm.sp_AutoStorageDocsHdrForRozCRM
	@ProcessID			as int,
	@ProcessNo			as int,
	@FiscalYear			as int,
	@DocStep			as int,
...
BEGIN TRY
	...
	SELECT ProcessID, ProcessNo, FiscalYear, SerialNo, DocDate, DocTime, DocStep, DocDesc, 
		StoreID, AcntCode, SaleTypeID, Discount, TaxOverWorthCost, TollOverWorthCost, TransportationIncome,Price, Amount, TotalLineDiscount,TransferSerialNo,VisitorAcntCode
	FROM inv.tblStorageDocsHdr 
	where ProcessID=@ProcessID AND ProcessNo=@ProcessNo AND FiscalYear=@FiscalYear and TransferSerialNo=@TransferSerialNo 
