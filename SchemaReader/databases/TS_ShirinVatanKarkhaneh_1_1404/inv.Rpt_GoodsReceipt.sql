...
-- =============================================
Create PROCEDURE [inv].[Rpt_GoodsReceipt]
	@ProcessID		Int = 192, -- Sale Order Process ID
	@ProcessNo		Int = Null,
...
Begin --============== S T A R T  C O D E ===================================================
	...
	SET @StrWhere = 'SH.ProcessID = ' + LTrim(RTrim(Str(@ProcessID))) + ' AND SH.ProcessNo = ' + LTrim(RTrim(Str(@ProcessNo)))
	...
	SELECT SH.ProcessID, SH.ProcessNo, SH.FiscalYear, SH.SerialNo, SH.DocDate, SD.StoreID, IsNull(S.StoreName,'''') StoreName,
		   SH.ReceiptType, SH.EnterTime, SH.AgreeNo, D2.FirstName + '' '' + LastName As DriverName, D.VehicleNo, SH.Count As CartonCount,
		   SH.LocationID, pub.funGetLocationName(SH.LocationID,' + LTrim(RTrim(@LangID)) + ') AS LocationName, SH.BaseSendID, BS.BaseSendName, 
		   SH.Weight As WaybillWeight, SH.TransportationType, SH.CarNo, SH.TrukNo, SH.TransporterID2 WaybillNo, SH.CCNo As BillNo, 
		   SD.LoadWeight, SD.EmptyWeight, [pub].[GetUserName](SH.SessionNo) AS UserName, SH.AcntCode, 
		   pub.GetCodeName(SH.AcntCode, 1) AS AcntName, SD.GoodsID, GD.GoodsName, SD.SendNo, SD.SubUnitID, UD.UnitName, 
		   SD.SubUnitQuantity, SD.NetWeight, SD.NetWeight /  ( case when SD.VirtualQuantity=0 then SD.SubUnitQuantity else  SD.VirtualQuantity end   ) As AverageWeight, SH.TransportationCost, 
		   ...
		   S7.UserSign As Signature5	,SD.VirtualQuantity	   
		   ,SD.BatchNo, B.BatchName
	...
	left join    inv.tblBatchDtl	B on B.BatchNo=	SD.BatchNo	and 		LanguageID=' + LTrim(RTrim(@LangID)) + '					 
