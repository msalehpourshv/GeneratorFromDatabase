...
-- Description   : آمار (یا وضعیت) درخواست کالاها - خلاصه
-- =============================================
CREATE PROCEDURE [cmr].[RptCMR_Statistics_Summary]
	@ProcessNo		TinyInt = 1,
	@SelectedGoods	Int = 0, 
	@SelectedAcnt1	Int = 0, 
	@SelectedAcnt2	Int = 0, 
...
Begin --============== S T A R T  C O D E ===================================================
	...
	SET @StrSelect = '
	SELECT	T.' + @CodeField + ', Sum(ConfirmQuantity) AS RequestedQty, 
	Sum(CanceledQty) AS CanceledQty, Sum(BoughtQty) AS BoughtQty, ' + @NameField + ', 
	IsNull([inv].[FunGetGoodsBarCode] (T.' + @CodeField + '), '''') BarCode
	FROM
	(
		SELECT	D.' + @CodeField + ', 
				D.ConfirmQuantity, 
		...
		WHERE   ' + @StrWhere + '
		GROUP BY D.' + @CodeField + ', D.ConfirmQuantity, D.ProcessID, D.ProcessNo, D.FiscalYear, D.SerialNo, D.DocRowNo
	) T 
