...
-- Description   :
-- =============================================
Create FUNCTION [pub].[funLockStatus]
(
...
BEGIN
	...
	DECLARE @LockProcessNo VARCHAR(3)
	...
	IF @ProcessID = 55 OR @ProcessID = 90 OR @ProcessID = 110 OR @ProcessID = 70
		BEGIN
			...
			declare @BSN	 as int
			declare @RsltProcessID	 as int
			declare @BatchNo as varchar(20)
			declare @ProductID as varchar(20)

			Select TOP 1 @BSN = BSN ,@BatchNo = BatchNo ,@ProductID = ProductID
			...
			BEGIN
					Select TOP 1 @Result = H.ProcessID , @LockProcessNo = H.ProcessNo
				...
				Where	H.ProcessID = 80 AND H.BatchNo = @BatchNo AND D.GoodsID=@ProductID
				ORDER By H.DocDate Desc,D.VolumeRowNo Desc
				IF @Result <> 0
			...
			IF @DocStep <= 1	
				BEGIN
					...
					Select TOP 1 @Result = DocStep, @RsltProcessID=@ProcessID ,@LockProcessNo = ProcessNo
					...
					IF @Result < '2'
						BEGIN
						if @BSN>0
							Select TOP 1 @Result = D.ProcessID , @LockProcessNo = D.ProcessNo
							from inv.tblStorageDocsDtl D 
							...
							And H.BaseSerialNo = D.BaseSerialNo
							Where	D.BaseProcessID = @ProcessID AND D.BaseProcessNo = @ProcessNo AND 
									D.BaseFiscalYear = @FiscalYear AND D.BaseSerialNo = @SerialNo AND BaseDocRowNo = @DocRowNo
							...
							Where	BaseProcessID = @ProcessID AND BaseProcessNo = @ProcessNo AND 
									BaseFiscalYear = @FiscalYear AND BaseSerialNo = @SerialNo AND (@ProcessID =70 or BaseDocRowNo = @DocRowNo)
						...
						END	
					ELSE
						Return 'ProcessID-' + LTRIM(STR(@RsltProcessID)) + '-' + 'DocStep-' +LTRIM(STR(@Result)) + '-' + LTRIM(STR(@LockProcessNo))  
				END
			ELSE			-- Confirm 
				BEGIN
					Select TOP 1 @Result = DocStep , @RsltProcessID=@ProcessID,@LockProcessNo = ProcessNo
	...
	ELSE IF @ProcessID = 75	-- Product_Send_Ret
		BEGIN
			...
			ELSE
				Return 'ProcessID-' + LTRIM(STR(@Result)) + '-' + LTRIM(STR(@LockProcessNo)) 
		END
	ELSE IF @ProcessID = 79
	BEGIN
	 
			Select TOP 1 @Result = a.SerialNo ,@LockProcessNo = a.ProcessNo
			From [inv].[tblStorageDocsDtl]  a
			inner join inv.tblInvTempReceiptDtl b on a.BatchNo=b.BatchNo and a.GoodsID=b.GoodsID
			Where	b.ProcessID = @ProcessID AND b.ProcessNo = @ProcessNo AND b.FiscalYear = @FiscalYear AND 
					b.SerialNo = @SerialNo 
	...
	ELSE IF @ProcessID = 240	
		BEGIN
			...
			END	
		END
	ELSE IF @ProcessID = 250 OR	 @ProcessID = 60
		BEGIN
			IF @DocStep = 1	
				BEGIN
					-- ==========								
	...
	ELSE IF @ProcessID = 255	--Asset Delivery Return
		...
		IF @DocStep = 1	
			...
			END
		END
	ELSE IF @ProcessID = 456	--Asset Return To Store
		BEGIN
				Select TOP 1 @Result = isnull(SerialNo,0) ,@LockProcessNo = ProcessID
				From [inv].[tblStorageDocsDtl] 
				Where	BaseProcessID = @ProcessID AND BaseProcessNo = @ProcessNo AND BaseFiscalYear = @FiscalYear AND 
						BaseSerialNo = @SerialNo AND BaseDocRowNo = @DocRowNo 
				ORDER BY DocStep Desc
