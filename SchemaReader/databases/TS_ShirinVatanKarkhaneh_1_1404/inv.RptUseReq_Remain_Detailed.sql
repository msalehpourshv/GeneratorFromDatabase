...
-- ==============================================
CREATE PROCEDURE [inv].[RptUseReq_Remain_Detailed]
	@ProcessNo			int = 1,
...
Begin --============== S T A R T  C O D E ===================================================
	...
	Set @StrSelect = '
	SELECT	T.*, pub.GetCodeName(T.AcntCode, 1) As AcntName, GD.GoodsName
	FROM
	(
		SELECT	D.ProcessID, D.ProcessNo, D.FiscalYear, D.SerialNo, D.DocRowNo, D.DocDate, D.AcntCode, D.GoodsID, D.ConfirmQuantity,
				(
		...
		WHERE   ' + @StrWhere + '
		GROUP BY D.ProcessID, D.ProcessNo, D.FiscalYear, D.SerialNo, D.DocRowNo, D.DocDate, D.AcntCode, D.GoodsID, D.ConfirmQuantity
	) T left join inv.tblGoodsDtl GD on GD.GoodsID = T.GoodsID
