...
-- Description   : 
-- =============================================
Create PROCEDURE [acc].[SpVchAssetTempEnter]
	@intVchNo				Int,
	@intDocStep				TinyInt,
    @strVchDate				Char(10),
...
BEGIN
	...
	While (@@Fetch_Status = 0)
		BEGIN			
				...
				SELECT TOP 1  @AssetAcntCode=case when isnull(AssetAcntCode2, '') <>'' then AssetAcntCode2 else AssetAcntCode end ,@CostAmount=CostAmount
				FROM  ast.tblAssetsDtl			 
				...
				SET @strRecDesc = ' بازگشت موقت دارایی ' + LTRIM(RTRIM(STR(@intSourceFiscalYear))) + '/' + LTRIM(RTRIM(STR(@intSourceSerialNo)))  + ' - ' + @AssetTitle + ' - به شماره پلاک  ' + @AssetPlaque
				...
				INSERT INTO acc.tblVoucherDtl
						(SerialNo,SourceProcessID,SourceProcessNo,SourceFiscalYear,SourceSerialNo,DocDate,IsAutoDoc,SessionNo,VchKind,RowNo,DocRowNo,
						 AcntCode,Debit,Credit,RecDesc,RecDesc2,SourceDocType) 
				VALUES	(@intVchNo,@intSourceProcessID,@intSourceProcessNo,@intSourceFiscalYear,@intSourceSerialNo,@strVchDate ,1,@SessionNo ,1,@intMaxRowNo ,@intMaxDocRowNo,
						 @ObverseAcntCode,0,@CostAmount,TS.pub.funChangeFarsiStrings(pub.funReverseForCrystal(@strRecDesc)),'',0 )	
