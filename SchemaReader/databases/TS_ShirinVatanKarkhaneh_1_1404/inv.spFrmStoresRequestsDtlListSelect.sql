...
-- Description   : 
-- =============================================
CREATE PROCEDURE [inv].[spFrmStoresRequestsDtlListSelect] 
	@DocDate	Char(10),
...
SELECT * FROM (
	SELECT DISTINCT	acc.funIsCodeClosed(Cd1.AcntCode) IsCodeClosed, Cd1.ProcessID, Cd1.ProcessNo, Cd1.FiscalYear, Cd1.SerialNo, Cd1.RowNo, Cd1.DocRowNo,
		Cd1.DocStep, Cd1.DocDate, Cd1.AcntCode, Cd1.GoodsID, Cd1.SubUnitID, Cd1.SubUnitQuantity, 
		Drv.GoodsQuantity, Cd1.DescDtl, Cd1.BaseProcessID,StoreID,
...
FROM	inv.tblStoresRequestsDtl AS Cd1 
	...
	(
		...
		From 
			(
				...
				Where ProcessID = 230 AND (@AcntCode IS NULL OR AcntCode = @AcntCode) AND (@GoodsID IS NULL OR GoodsID = @GoodsID) AND 
	...
	WHERE Drv.GoodsQuantity > 0  AND Cd1.DocDate<=@DocDate AND
		 (@AcntCode IS NULL OR  Cd1.AcntCode = @AcntCode) AND 
		 (@BaseSerialNo IS NULL OR (Cd1.SerialNo=@BaseSerialNo AND Cd1.FiscalYear=@BaseFiscalYear)) 
