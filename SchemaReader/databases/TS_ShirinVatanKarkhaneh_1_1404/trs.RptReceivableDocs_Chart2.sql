...
-- Viewed By	 : 
-- Last Modified : 1389/03/02
-- Description	 : <Receivable Documents Chart>
-- ----------------------------------------------
-- نمودار اسناد دریافتنی
-- ==============================================
Create PROCEDURE [trs].[RptReceivableDocs_Chart2]
	@ProcessNo		int = 1,
...
Begin   
	...
	If @ResultCategory = 1 or  @ResultCategory = 4 
	Begin
		-- موجود واگذار نشده --
		If @IncludeReceive = 1 
		Begin
	...
	If @ResultCategory = 2  or  @ResultCategory = 4 
	Begin
	...
	-- Category <3> (Returned to Owner Cheques) --
	 If @ResultCategory = 3   or  @ResultCategory = 4 
	Begin
...
------------------------------------------------------------------------------------------------------
	...
	SET @StrSelect = '
		...
		--	and (D.ProcessID IN (' + @StrProcessID + ')) 
			and (D.ProcessNo = ' + LTRIM(STR(@ProcessNo)) + ')
			and	(D.PayTypeID IN (6,26)) 
			and ' + @StrWhere

 If @ResultCategory <> 4 
 	SET @StrSelect = 	@StrSelect + ' and (D.ProcessID IN (' + @StrProcessID + '))  '
