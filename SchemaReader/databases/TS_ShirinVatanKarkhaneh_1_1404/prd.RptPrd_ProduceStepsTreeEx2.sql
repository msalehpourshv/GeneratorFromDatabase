...
-- Create date   : 1389/09/18
-- Viewed By	 : 
-- Last Modified : 1389/10/12
-- Last Modifier : TakroSystem\Zia
-- Description   : <مراحل تولید محصولات - درختی 3>
-- =================================================================
CREATE PROCEDURE [prd].[RptPrd_ProduceStepsTreeEx2]
	@SelectedProds	Int = 0,
	@SelectedGoods	Int = 0,
...
BEGIN
	...
	select R.GoodsGroupID, N.GoodsGroupName,
			isnull(sum(B0.Balance), 0) Balance, 
			isnull(sum(B1.Balance), 0) Balance1,
			isnull(sum(B2.Balance), 0) Balance2,
			isnull(sum(B3.Balance), 0) Balance3,
			isnull(sum(B4.Balance), 0) Balance4
	from #tbl_Result A
			inner join inv.tblGoodsGroupsGoodsListDtl R on R.GoodsID = A.ProductID
			inner join inv.tblGoodsGroupsDtl N on N.GoodsGroupID = R.GoodsGroupID
			...
			--left join inv.tblGoodsDtl G on G.GoodsID = A.ProductID
