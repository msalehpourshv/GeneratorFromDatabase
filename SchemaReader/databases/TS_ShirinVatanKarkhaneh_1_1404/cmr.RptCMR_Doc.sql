...
-- Viewed By	 : 
-- Last Modified : 1393/06/25
-- Last Modifier : TakroSystem/ZiA
-- Description   : برگ درخواست کالا
-- =============================================
Create PROCEDURE [cmr].[RptCMR_Doc]
...
Begin --============== S T A R T  C O D E ===================================================
	...
	-- S E L E C T ------------------------------------------------------------
	SELECT	D.*,isnull(tDD.DepartmentName,'') DepartmentName, H.DocDesc, [pub].[funGetGoodsName](D.GoodsID,@LanguageID) GoodsName,
			[inv].[funGetGoodsRemain](NULL,NULL,D.FiscalYear,NULL,NULL,NULL,D.GoodsID,'',D.DocDate,0) GoodsRemainAll,
			[inv].[funGetGoodsRemain](NULL,NULL,NULL,NULL,NULL,@DefaultStoreID,D.GoodsID,'',H.DocDate,0) AS GoodsRemain1,
			[inv].[funGetGoodsRemain_ByInventoryType](NULL,NULL,NULL,NULL,NULL,0,D.GoodsID,'',H.DocDate,0) AS GoodsRemainPrimeryMaterial,
		    ...
		    S8.UserSign As Signature5,[inv].[FunCalcGoodsWeight] (D.GoodsID) GoodsWeight
	...
	INNER JOIN inv.tblGoodsDtl G ON G.GoodsID = SUBSTRING(D.GoodsID,@str_Goods+1, @str_GoodsSum) AND G.PartNumber= @UnitPart AND G.LanguageID = @LanguageID
	LEFT  JOIN inv.tblUnitsDtl U ON U.UnitID = D.SubUnitID AND U.LanguageID = @LanguageID
	LEFT  JOIN prs.tblDepartmentsDtl tDD ON H.SenderDepartmentID = tDD.DepartmentID
