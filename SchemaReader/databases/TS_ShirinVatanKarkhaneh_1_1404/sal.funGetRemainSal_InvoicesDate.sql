...
GO
Create FUNCTION [sal].[funGetRemainSal_InvoicesDate]
(
	@ProcessID int=90
	,@ProcessNo  int=1
	,@FiscalYear  int=93
...
Declare @Ret Varchar(10);  
 ...
 select @Ret=
  pub.funFarsiDateAddDays('day',@VchDate,
 CONVERT(int,ISNULL( 
sum(pub.funFarsiDateDiff('day',pub.funFarsiDate(GETDATE() ),SettlementDate)*Remain)/sum(Remain),0
)
))
from (
