...
-- ==============================================
Create PROCEDURE [cmr].[RptBuyOrder_Remain_Summary]
	@ProcessNo			Int = 1,
...
Begin --============== S T A R T  C O D E ===================================================
	...
	SET @PID_REQ = '160';		-- Buy Request
	SET @PID_REQ_CNL = '165';	-- BuyRequest Cancel
	...
	IF @bolShowAllOrders = 'True'
		SET @StrShowAllOrders = '
			...
			WHERE (T.BoughtQty < T.RequestedQty - T.CanceledQty)' 
	ELSE
		SET @StrShowAllOrders = ''			
	---------------------------------------------------------------------------
			...
			(
				SELECT	D.FiscalYear, D.SerialNo, D.DocDate, D.AcntCode,
						...
						(
							...
							WHERE	CNL.BaseProcessID =D.ProcessID AND CNL.BaseProcessNo=D.ProcessNo AND CNL.BaseFiscalYear=D.FiscalYear AND CNL.BaseSerialNo=D.SerialNo AND CNL.BaseDocRowNo=D.DocRowNo AND CNL.ProcessID=' + @PID_REQ_CNL + '
						) AS CanceledQty,
				...
				WHERE   ' + @StrWhere + '
				GROUP BY D.ProcessID, D.ProcessNo, D.FiscalYear, D.SerialNo, D.DocRowNo, D.DocDate, D.AcntCode, D.ConfirmQuantity, OH.AgreeNo
			) T ' + @StrShowAllOrders + '
			ORDER BY ' + @SortFields
			---------------------------------------------------------------------------
	...
	Begin
			...
			Where T.RequestedQty - T.CanceledQty > 0
			ORDER BY ' + @SortFields
			---------------------------------------------------------------------------
