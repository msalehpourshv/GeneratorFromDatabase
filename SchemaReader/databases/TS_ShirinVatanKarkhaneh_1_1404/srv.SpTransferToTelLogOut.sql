...
-- Description   : 
-- =============================================
CREATE PROCEDURE [srv].[SpTransferToTelLogOut]
...
BEGIN
    ...
    WHERE Incoming=0 AND CallDate >= @CurrentDate --AND Internal=' ' AND Ext=' '
    --AND a.CallDate >=@StartDate AND a.CallDate <=@EndDate
    
    EXCEPT
    
    SELECT EventID FROM pub.tblTelLog
    WHERE  DocDate >= @CurrentDate 
	 ...
	 IF (SELECT COUNT(*) from #tblTempTelLog)>0
		BEGIN
			...
			WHERE EventID IN (SELECT EventID FROM #tblTempTelLog )--Incoming=1 AND Internal=' ' AND Ext=' ' --AND EventID >  & funMaxOfColumn() & _
			ORDER BY EventID
