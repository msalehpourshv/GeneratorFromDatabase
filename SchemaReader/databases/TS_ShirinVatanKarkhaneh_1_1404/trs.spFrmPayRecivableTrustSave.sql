...
Create PROCEDURE [trs].[spFrmPayRecivableTrustSave] 
 @ProcessID		tinyint,
 @ProcessNo	    tinyint,
 @FiscalYear	smallint,
 @SerialNo      int,
...
begin
	...
	BEGIN
		...
		SET @strMsgText=TS.pub.funGetMessages(12069,@LanguageID)
	...
	BEGIN
		...
		SET @strMsgText=TS.pub.funGetMessages(12070,@LanguageID)
		Raiserror (@strMsgText,16,1,@LocationID)
		Return
	END

    -------------------------------------------
	IF @PayTypeID=16 OR @PayTypeID = 31   OR @PayTypeID = 34
	BEGIN
		DECLARE @TempProcessID Tinyint

		SELECT TOP 1 @TempProcessID= ProcessID 
		FROM trs.tblPayDtl
		WHERE PayTypeID IN (@PayTypeID) AND
		...
		BEGIN
			Close Cursor_PayDtl;
			Deallocate Cursor_PayDtl; 
			--چک به شماره %s مسترد شده است
			SET @strMsgText=TS.pub.funGetMessages(12071,@LanguageID)
