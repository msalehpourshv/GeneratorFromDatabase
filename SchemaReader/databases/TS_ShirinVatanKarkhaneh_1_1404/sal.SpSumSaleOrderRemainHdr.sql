...
-- Description   : 
-- =============================================
CREATE PROCEDURE  [sal].[SpSumSaleOrderRemainHdr] 
...
BEGIN
	...
	IF @DocStep = 0
		...
		set @strSelectHdr= '
			...
			VALUES(180 , ' + ltrim(str(@ProcessNo)) +' ,' +LTRIM(str(@FiscalYear)) +' , ' + ltrim(str(@SerialNo)) +' ,' + ltrim(str(@DocStep)) + ' ,''' +@DocDate +''' ,''' + @AcntCode + ''',0 
			, 0 , 0 , 0 , 0 , '''' , '''' , '''' , '''' , '''+ @SaleTypeID +''',
