...
-- ==============================================
CREATE PROCEDURE [sal].[RptSale_Sale_Doc_New]
	@ProcessID		Int = 90,
	@ProcessNo		Int = 1,
	@FiscalYear		Int = 97,
	@SerialNo		Int = 1,
...
BEGIN --============== S T A R T  C O D E =======================================
	...
	-- SELECT Clause ----------------------------------------
	Set @StrSelect1  = '
	SELECT	D.*, 0 Discount2, H.Discount AS DiscountHdr, H.Discount2 AS Discount2Hdr, U.UnitName, U3.UnitName UnitName3, 
			W2.UnitName As SubUnitName,H.TransporterID, [sal].[GetTransporterNameAry] (H.TransporterID) As TransporterNameAry,
			IsNull(W.UnitName,'''') UnitName2, H.DiscountPercent AS DiscountPercentHdr, H.DocDesc, S.StoreName, 
			...
			GH.TechnicalSpecifications, GH.MiscSpecifications, '''' HdrAcntName,
			IsNull(F.AcntName,'''') AcntName,
	...
	FROM	' + @SD + ' D 
				...
				LEFT JOIN inv.tblGoodsDtl GD ON GD.GoodsID=SUBSTRING(D.GoodsID3,' + ltrim(rtrim(STR(@str_Goods+1))) + ',' + ltrim(rtrim(STR(@str_GoodsSum))) + ') AND GD.PartNumber = 2
