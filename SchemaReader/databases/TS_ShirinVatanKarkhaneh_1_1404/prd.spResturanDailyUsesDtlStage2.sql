...
-- Create date   : 92/01/20
-- Viewed By	 : 
-- Last Modified : 
-- Description   : 
-- =============================================
CREATE PROCEDURE [prd].[spResturanDailyUsesDtlStage2]
...
DECLARE @PureQty AS FLOAT
	...
	-------------- ���� ���� ����� ���� ���� �� ���� ---------------
	--SELECT r.* FROM
	--	(SELECT h.ProductID,h.ProductCount,d.GoodsID,d.GoodsQuantity,g.UnitID
	...
	--WHERE h.IsDefault='True' AND h.ProductID='20602')r
	---------------------------------------------------------------
	...
	IF @PureQty>0
		...
		select 80, 1, @FiscalYear, @maxSerialNo, @RowNo + 1 RowNo, @DocRowNo1+1 DocRowNo, 
					0, 1, @ToDate, @StoreID2, 'True', +1, '', @AcntCode, '', '', @GoodsID,UnitID SubUnitID,(@PureQty) SubUnitQuantity,
					(@PureQty) GoodsQuantity,0 QtyRemain,0 GoodsAmount,0 AmntRemain,0 AtomAmount,0 GoodsPrice,'' DescDtl,
					0 BaseProcessID,0 BaseProcessNo,0 BaseFiscalYear,0 BaseSerialNo,0 BaseDocRowNo,'' BaseDocType,0 AgreeNo,
				    '' BatchNo,0 DiscountPercentDtl,0 DiscountDtl,'' BaseDocDate,0 VirtualQuantity,'False' IsReward,
	...
	SELECT @DocRowNo1 = isnull(MAX(DocRowNo),0)
		...
		select 90, 11, @FiscalYear, @maxSerialNo, @RowNo+1 RowNo,@DocRowNo1 DocRowNo, 
			   0, 3, @ToDate, @StoreID2, 'True', -1, '', @AcntCode, '', '', @GoodsID,f.UnitID SubUnitID,@Qty SubUnitQuantity,
