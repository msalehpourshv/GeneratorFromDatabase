...
-- Last Modified : 
-- Last Modifier : 
-- Description	 : گزارش سفارش فروش
-- ==============================================
Create PROCEDURE [sal].[RptSaleOrderList]
...
set @StrWhereD=' AND ( 1=0 '
  ...
  IF (@ProcessIDRet1 =1)	
		SET @StrWhereD = @StrWhereD + ' or  (D.ProcessID=185 and D.BaseProcessID=180 )'
...
--select @StrWhereD
	...
	set @StrSelect='   insert into #tblTemp select distinct top '+ str(@Cnt) +'   '+ @Group1 +' ,'+ @Group1 +' 
		...
		ON D.ProcessID = H.ProcessID And D.ProcessNo = H.ProcessNo And D.FiscalYear = H.FiscalYear And D.SerialNo = H.SerialNo)D where  '+ @Group1 +'<>''''	and  1=1 '+@StrWhereD+'	'
	else
	set @StrSelect='   insert into #tblTemp select distinct top '+ str(@Cnt) +'   '+ @Group1 +' ,'+ @Group1 +' 
...
set  @Columns2= SUBSTRING(@Columns2,2,LEN(@Columns2)-1)
	...
	SUM(GoodsPrice*GoodsQuantity+TaxOverWorthCostDtl+TollOverWorthCostDtl)over (partition by D.ProcessID,D.ProcessNo,D.FiscalYear,D.SerialNo) Balance,
	 -1 EnterKind,LocationID LocationIDDtl,
...
------------------------------------------------------------------------------------------------
	...
	SET @StrSelect = ' insert into #RptTable
		select  
			...
			0 as DiscountAll,
			0 as GoodsPriceHdr,
			0 as GoodsPriceAfter,
			TransportationCost,
			Discount+Discount2 DiscountHdr,		
			ISNULL(  GoodsPrice*GoodsQuantity+TaxOverWorthCostDtl+TollOverWorthCostDtl , 0) as GoodsPriceDtl, 
			 Price-TotalLineDiscount Balance , case when D.ProcessID=180 then -1 else 1 end EnterKind,LocationID LocationIDDtl,
			D.*  
...
select Name1 [کد ], '+ @Name +' [نام],'+@Columns2+' ,'+@ColumnsSum+' [مجموع]   
   from
		...
		( 
			...
			from
			(
				select '+@Group2+' Name1 , '+@Group1+' Name2 , '+ @StrType +'  Price  from  #RptTable D -- sal.tblSaleOrderDtl 
