...
-- Description   : 
-- =============================================
CREATE PROCEDURE [inv].[sp_api_VaraNegar_AutoSaleHdr]
@ProcessNo as Int,
@FiscalYear as Int,
@DocStep as tinyint,
@DocDate as CHAR(10),
...
BEGIN TRY
    ...
    Else
		SELECT @DiscountAcntCode =SaleDiscountAcntCode FROM inv.tblStores WHERE StoreID <>'' AND StoreID =@StoreID
	...
	SELECT 90 ProcessID ,@ProcessNo ProcessNo, @FiscalYear FiscalYear,@DocDate DocDate, @maxSerialNo SerialNo,@AcntCode AcntCode,@StoreID StoreID,@SaleTypeID SaleTypeID
END TRY
BEGIN CATCH
