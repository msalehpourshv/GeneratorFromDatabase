...
Create FUNCTION [sal].[funGetLineDiscount]
(
...
Begin -- === S T A R T ===========================================
	...
	-- ==============================================================
	SELECT TOP 1 @Result = CASE WHEN DiscountPercent > 0 THEN 
	...
	WHERE	(Discount > 0 Or DiscountPercent > 0) 
			AND D.ProcessID = 200 AND ((FromDate <= @DocDate AND ToDate >= @DocDate)) 
			AND ((ForQty > 0 AND ForQty <= @GoodsQuantity)
			OR  (@GoodsPrice >= FromAmount AND @GoodsPrice <= ToAmount) 
			OR  (@GoodsQuantity >= FromQty And @GoodsQuantity <= ToQty)) 
	...
	ORDER BY FromDate Desc,D.SerialNo Desc ,DocRowNo ,ForQty Desc, FromQty Desc 
	-- ==============================================================
