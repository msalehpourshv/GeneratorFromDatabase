...
--Description   : 
--================================================
CREATE PROCEDURE [inv].[SpVch_CreatDoc_Baskul]
	  @ProcessID AS INT,
	  @ProcessNo AS INT,
...
BEGIN
  BEGIN TRY
	...
	DECLARE  aa_curs CURSOR FOR   
	   SELECT  VchNo, DocDate, ProcessID, ProcessNo, FiscalYear, SerialNo
	   FROM   inv.tblBaskulSalesHdr
	   WHERE 1 = 1 AND ProcessID = @ProcessID AND ProcessNo = @ProcessNo AND 
	 	     DocDate >= @tmpFromDocDate AND DocDate <= @tmpToDocDate AND 
			 SerialNo >= @tmpFromSerialNo AND SerialNo <= @tmpToSerialNo AND 
	  ...
	  BEGIN
		  ...
		  BEGIN
			...
			EXEC [acc].[SpVch_CreateDoc]   
					...
					@strHdrTblName			= 'inv.tblBaskulSalesHdr',  
