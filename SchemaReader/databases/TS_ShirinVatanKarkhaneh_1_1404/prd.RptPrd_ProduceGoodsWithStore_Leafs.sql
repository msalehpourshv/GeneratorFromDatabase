...
-- =========== TS-QC:UPDATED ====================
-- Author		 : 
-- Create date   : 1398/10/17
-- Viewed By	 : 
-- Last Modified : 
-- Last Modifier : 
-- Description   : <لیست مواد اولیه تعریف شده برای تولید یک محصول - فقط برگها>
-- =================================================================
CREATE PROCEDURE [prd].[RptPrd_ProduceGoodsWithStore_Leafs]
...
BEGIN
	...
	declare csr_Products cursor scroll for
		select ProductID,StoreID, ProductQty
		from #tbl_RptPrd_ProduceGoods_Leafs_Products
	open csr_Products;

	fetch ABSOLUTE @current from csr_Products into @MyProductID,@DefaultStoreID, @MyProductQty;
	...
	begin
		...
		begin
			...
			select	D.GoodsID,D.DefaultStoreID, (D.GoodsQuantity * @MyProductQty / H.ProductCount)
			...
			where	(D.ProductID = @MyProductID) AND 
			       ((Select  Count(* ) from prd.tblFormulasDtl bb where bb.ProductID=D.GoodsID )=0 )
		end
		else
		begin
	...
	if (@CalcBalance = 1) 
		set @blc = '
		(
			select isNull(sum(GoodsQuantity * EnterKind), 0) GoodsRemain
	...
	set @StrSelect = '
		select	T.GoodsID,T.StoreID ,  [pub].[funGetGoodsName](T.GoodsID, 1) As GoodsName, isnull(sum(T.GoodsQty), 0) AS GoodsQuantity, 
		' + @blc + ' AS GoodsRemain	,gg.UnitID,UnitName 	
		...
		where ' + @StrWhere + '
		group by T.GoodsID,T.StoreID, G.GoodsName,gg.UnitID,UnitName
