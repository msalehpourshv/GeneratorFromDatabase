...
-- =============================================================================================
-- =============================================================================================
CREATE PROCEDURE sal.RptSale_PriceList2_SHV
	@SelectedGoods	Int = 0,
	@PriceFr		BigInt = Null,
	@PriceTo		BigInt = Null,
...
BEGIN --============== S T A R T  C O D E ===================================================
   ...
   set @SalePriceTax = 0
	...
	SET @ShowGoodsImage = 1
	-- =========================================================================================================
	SET @StrSelect = '
		SELECT T.*, U.UnitName, IsNull([inv].[FunGetGoodsBarCode] (T.GoodsID), '''') BarCode, G.TechnicalSpecifications, G.TechnicalNo, 
		                G.ExtraField1, G.ExtraField2, G.ExtraField3, G.ExtraField4, G.ExtraField5, GoodsLength, GoodsWidth, GoodsHeight, GoodsWeight, PureWeight GoodsPureWeight,
				...
				) BalanceQty ,IsNull(UPI.UParams,'''') UserPrice' + 
				Case When @ShowGoodsImage = 1 Then ', Cast(IsNull(GI1.GoodsImage, '''') As Image) GoodsImage ' Else ', Cast('''' As Image) GoodsImage' End + ',
				CASE WHEN GI1.GoodsImage IS NULL THEN ''NULL'' WHEN DATALENGTH(GI1.GoodsImage) = 0 THEN ''Empty'' ELSE ''HasData'' END ImageStatus,
				b.UnitName UnitName2,UnitValue,MainUnitValue ,Case when G.ContainTax=1 then Round(('+cast (@Value as nvarchar(500))+'* IsNull(T.SalePrice, 0))/100 ,2) + T.SalePrice  else 0 end  as SalePriceTax 
		FROM
		(
			SELECT	' + @FldGoodsID + ', ' + @price + ' as SalePrice,PriceDate,' + @FldGoodsName + ' GoodsName, S.UserPriceID, S.UserPrice UserPrice2, DescDtl
			FROM	' + @StrFrom + '
			WHERE	' + @StrWhere + '
		) T 
		...
		INNER JOIN inv.tblGoodsDtl GD ON GD.GoodsID = SUBSTRING(T.GoodsID,' + ltrim(rtrim(STR(@str_Goods+1))) + ',' + ltrim(rtrim(STR( @str_GoodsSum))) + ')
