...
-- Description   : 
-- =============================================
Create FUNCTION [sal].[funGetSaleOrderGoodsRemain_Store]
(	
	@GoodsID		Varchar(20),
	@StoreID		Varchar(20),
	@DocDate		Char(10),
...
BEGIN
			...
			IF @SalRet_RetToSalOdr = 'True'
				BEGIN
					...
					WHERE BaseProcessID = 180 AND
						  ...
						  DocDate <= @DocDate AND 
						  GoodsID = @GoodsID --AND (@StoreID IS NULL OR (StoreID = @StoreID))
					 ) B
	...
	ELSE
		BEGIN
  			...
  			-- ======
			SELECT @QtyRemain = ISNULL(SUM(S.GoodsQuantity),0) --Case When @SorHasFirstConfirm = 'True' Then ISNULL(SUM(ConfirmQuantity),0) Else ISNULL(SUM(GoodsQuantity),0) End
			FROM sal.tblSaleOrderDtl S 
			INNER JOIN sal.tblSaleOrderHdr A
		    ON A.ProcessID=S.ProcessID AND A.ProcessNo=S.ProcessNo AND A.FiscalYear=S.FiscalYear AND A.SerialNo=S.SerialNo
			WHERE S.ProcessID = 180 AND
			      IsSold='False' AND
				  S.DocDate <= @DocDate AND 
				  S.FiscalYear= @FiscalYear AND
				  GoodsID = @GoodsID AND  ((S.StoreID = '' and A.StoreID = '') OR @StoreID IS NULL OR (S.StoreID = @StoreID or A.StoreID = @StoreID))  AND 
			...
			WHERE S.BaseProcessID = 180 AND
			      IsSold='False' AND
				  S.DocDate <= @DocDate AND 
				  S.BaseFiscalYear= @FiscalYear AND
				  GoodsID = @GoodsID AND  ((S.StoreID = '' and A.StoreID = '') OR @StoreID IS NULL OR (S.StoreID = @StoreID or A.StoreID = @StoreID))  AND 
				  (@UserPriceID =0 or UserPriceID=@UserPriceID)
