...
-- Description   : 
-- =============================================
CREATE PROCEDURE  [sal].[SpSumSaleOrderRemainDtl] 
...
BEGIN
	...
	IF @DocStep = 0
		...
		WHERE ProcessID=180 AND SerialNo=@SerialNo AND FiscalYear = @FiscalYear AND ProcessNo=@ProcessNo
		...
		set @strSelectDtl= 'INSERT INTO [sal].[tblSaleOrderDtl]
			(ProcessID, ProcessNo, FiscalYear, SerialNo, RowNo, DocRowNo, DocStep, DocDate,
			 ...
			 SaleTypeID, IsReward, SubUnitPrice, AutoOrder, TransferSerialNo)
				SELECT  180 ProcessID, ' + ltrim(rtrim(str(@ProcessNo))) + ' ProcessNo, ' + ltrim(rtrim(str(@FiscalYear))) + ' FiscalYear,'
				...
				'+ltrim(rtrim(str(@GoodsPrice)))+' GoodsPrice, '''' DescDtl,0 BaseDocType ,0 BaseProcessID ,0 BaseProcessNo,
				0 BaseFiscalYear,0 BaseSerialNo,0 BaseDocRowNo, '''' AgreeNo, '''' OrderDate, '''' DeliveryDate,
				'''' VisitorAcntCode, ''' + @SaleTypeID + ''', 0 IsReward, 0 SubUnitPrice, 1 AutoOrder, 0'
