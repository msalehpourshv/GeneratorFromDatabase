...
-- =============================================================================================
-- =============================================================================================
CREATE PROCEDURE sal.RptSale_PreSale_Doc_SHV
	@ProcessID		Int = 240,
	@ProcessNo		Int = 1,
	@FiscalYear		Int = 95,
	@SerialNo		Int = 1,
...
----  برای نمایش مانده حساب ارزی بر اساس ارز
	...
	if 1 = 1
	begin
		set @StrSelectIMGField=', Cast(GI.GoodsImage As Image) GoodsImage'
	...
	SELECT	D.*,
			...
			pub.funFarsiDate(GETDATE()) PrintDate '+@StrSelectIMGField +', IsNull(CT.ISOCode,'''') CurrencyTypeISOName, 
			pub.funChangeDate_PersianToGergorian(D.DocDate) GeorgDocDate, H.IsSubUnit, H.Discount AS DiscountHdr, H.Discount2 AS Discount2Hdr, 
			U.UnitName,W2.UnitName As SubUnitName, H.TransporterID, [sal].[GetTransporterNameAry] (H.TransporterID) As TransporterNameAry,
			...
			[pub].[funGetGoodsName](D.GoodsID,' + LTrim(RTrim(Str(@LanguageID))) + ') GoodsName, GD.GoodsName2,
			GD.GenericName, pub.funSplitString(GenericName,''|'', 1) GoodsName_EN, pub.funSplitString(GenericName,''|'', 2) GoodsName_FA,
			GH.ExtraField1,GH.ExtraField2,GH.ExtraField3,GH.ExtraField4,GH.ExtraField5,GH.GoodsPrice, '''' VehicleNo,
	...
	Set @StrSelect2  = '								 
			...
			,isnull(DR.DescRetSaleID,'''') DescRetSaleID , isnull(DR.DescRetSaleName,'''') DescRetSaleName, '+LTrim(RTrim(str(@ShowDiscount)))+' ShowDiscount,H.CurrencyDiscount,
			H.CurrencyRate, H.CurrencyTypeID, [pub].[funGetCurrencyTypesName] ( H.CurrencyTypeID,'+ LTrim(RTrim(str(@LanguageID))) +') CurrencyTypesName
	INTO	##tbl_Tmp
	FROM	' + @SD + ' D 
	...
	INNER JOIN inv.tblGoodsDtl       GD ON GD.GoodsID = D.GoodsID AND GD.LanguageID = ' + LTrim(RTrim(Str(@LanguageID))) + '
