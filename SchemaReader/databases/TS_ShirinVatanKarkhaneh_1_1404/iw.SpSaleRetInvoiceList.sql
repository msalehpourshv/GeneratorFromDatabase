...
-- Last Modifier : 
-- ----------------------------------------------
-- Description	 : < مشخصات فاکتورهای برگشت فروش  >
-- ==============================================
Create PROCEDURE iw.SpSaleRetInvoiceList
	@UserID			int,
...
DECLARE @StrSelect		NVarChar(MAX);
	...
	select  Count(*)over () TotalCount, ProcessID, ProcessNo, FiscalYear, SerialNo,DocDate, AcntCode,
		...
		,CreditCardDiscount,AutoDiscont,AutoDiscont2,DiscountTaxOverWorth,TaxOverWorthCost,TollOverWorthCost
		,isnull( (select Sum(Amount) from trs.tblPayDtl PD
					inner join  trs.tblPayHdr PH
					on PH.ProcessID=PD.ProcessID
					And PH.ProcessNo=PD.ProcessNo
					And PH.FiscalYear=PD.FiscalYear
					And PH.SerialNo=PD.SerialNo
					where PH.BaseProcessID=H.ProcessID
					and PH.BaseProcessNo=H.ProcessNo
					and PH.BaseFiscalYear=H.FiscalYear
					and PH.BaseSerialNo=H.SerialNo),0) Payment
