...
-- Create date   : 1392/01/19
-- Viewed By	 : 
-- Last Modified : 
-- Last Modifier : 
-- Description	 :   گزارش برگه هاي ورود صورت وضعيت  - فروشنده
-- =============================================
CREATE PROCEDURE [trn].[RptCnt_SyncSaler_Docs]
	@FromSerialNo		INT ,
	@ToSerialNo			INT,
	@FromFiscalYear		INT,
	@ToFiscalYear		INT ,
...
-- ================ WHERE ===========================
	...
	IF (@ContractorID IS NOT null)
		SET @StrWhere = @StrWhere + ' AND h.ContractorAcntCode = ''' + @ContractorID + ''''
...
-- ================ SELECT ===========================
	...
	SELECT 
		...
		cnt.funSalePrice(h.SerialNo,h.ProcessID,h.ProcessNo,h.FiscalYear,1) as SalrPrice1,
		cnt.funGetPureWeight(h.SerialNo,h.ProcessID,h.ProcessNo,h.FiscalYear,2) as wheight2,
		cnt.funSalePrice(h.SerialNo,h.ProcessID,h.ProcessNo,h.FiscalYear,2) as SalePrice2,
		cnt.funGetPureWeight(h.SerialNo,h.ProcessID,h.ProcessNo,h.FiscalYear,3) as wheight3,
		cnt.funSalePrice(h.SerialNo,h.ProcessID,h.ProcessNo,h.FiscalYear,3) as SalePrice3,
		cnt.funGetPureWeight(h.SerialNo,h.ProcessID,h.ProcessNo,h.FiscalYear,4) as wheight4,
		cnt.funSalePrice(h.SerialNo,h.ProcessID,h.ProcessNo,h.FiscalYear,4) as SalePrice4,
		h.*, pub.GetCodeName(h.ContractorAcntCode,' + LTRIM(STR(@LangID)) + ') AS ContractorName,
		pub.GetGoodsName(h.GoodsID,' + LTRIM(STR(@LangID)) + ') AS GoodsName,
	...
	WHERE ' + @StrWhere + '
	ORDER BY h.ContractorAcntCode,h.FactoryAcntCode '
