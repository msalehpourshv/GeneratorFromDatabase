...
-- Create date   : 1389/07/11
-- Viewed By	 : 
-- Last Modified : 1390/08/21
-- Last Modifier : TakroSystem\Zia
-- Description	 : تجزیه سنی - بستانکاران
-- ===============================================
CREATE PROCEDURE [acc].[RptAcc_AgeAnalysis2]
...
declare @CreditRemainFr as bigint;
declare @CreditRemainTo as bigint;
...
declare @PrevCreditRemainFr as bigint;
declare @PrevCreditRemainTo as bigint;
...
declare @CreditRemain as bigint;
...
Begin
	...
	-------------------------------------
	SELECT	T.*, (CreditRemainFr - DebitSum + CreditSum) CreditRemainTo, 
	...
	update #tblResult
	set #tblResult.CreditRemainFr = (N.CreditSum - N.DebitSum)
	...
	begin
		-- clear debtors from list
		select top 1 @CreditRemain = CreditRemainTo
		from #tblResult
		...
		select @PrevMonthCode='00',@PrevCreditRemainFr=0,@PrevCreditRemainTo=0,@PrevDebitSum=0,@PrevCreditSum=0,@PrevMonth00=0, @PrevMonth01=0, @PrevMonth02=0, @PrevMonth03=0, @PrevMonth04=0,@PrevMonth05=0,@PrevMonth06=0,@PrevMonth07=0,@PrevMonth08=0,@PrevMonth09=0,@PrevMonth10=0,@PrevMonth11=0,@PrevMonth12=0
		...
		fetch next from crs_AcntRecords into @AcntCode, @MonthCode, @CreditRemainFr, @DebitSum, @CreditSum, @CreditRemainTo, @Month00,  @Month01, @Month02, @Month03, @Month04, @Month05, @Month06, @Month07, @Month08, @Month09, @Month10, @Month11, @Month12

		while (@@fetch_status = 0)
		begin
			-- previous credit for first month
			if (@MonthCode = '01')
				set @PrevMonth00 = @CreditRemainFr

			if (@CreditRemainFr < 0) and (@MonthCode > '01')
				set @DebitSum = @DebitSum - @CreditRemainFr
						 
			-- remain of previous year --
			if (@DebitSum > @PrevMonth00)
			begin
				set @Month00 = 0
				set @DebitSum = @DebitSum - @PrevMonth00
			end
			else
			begin
				set @Month00 = @PrevMonth00 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 1 --
			if (@DebitSum > @PrevMonth01)
			begin
				set @Month01 = 0
				set @DebitSum = @DebitSum - @PrevMonth01
			end
			else
			begin
				set @Month01 = @PrevMonth01 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 2 --
			if (@DebitSum > @PrevMonth02)
			begin
				set @Month02 = 0
				set @DebitSum = @DebitSum - @PrevMonth02
			end
			else
			begin
				set @Month02 = @PrevMonth02 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 3 --
			if (@DebitSum > @PrevMonth03)
			begin
				set @Month03 = 0
				set @DebitSum = @DebitSum - @PrevMonth03
			end
			else
			begin
				set @Month03 = @PrevMonth03 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 4 --
			if (@DebitSum > @PrevMonth04)
			begin
				set @Month04 = 0
				set @DebitSum = @DebitSum - @PrevMonth04
			end
			else
			begin
				set @Month04 = @PrevMonth04 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 5 --
			if (@DebitSum > @PrevMonth05)
			begin
				set @Month05 = 0
				set @DebitSum = @DebitSum - @PrevMonth05
			end
			else
			begin
				set @Month05 = @PrevMonth05 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 6 --
			if (@DebitSum > @PrevMonth06)
			begin
				set @Month06 = 0
				set @DebitSum = @DebitSum - @PrevMonth06
			end
			else
			begin
				set @Month06 = @PrevMonth06 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 7 --
			if (@DebitSum > @PrevMonth07)
			begin
				set @Month07 = 0
				set @DebitSum = @DebitSum - @PrevMonth07
			end
			else
			begin
				set @Month07 = @PrevMonth07 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 8 --
			if (@DebitSum > @PrevMonth08)
			begin
				set @Month08 = 0
				set @DebitSum = @DebitSum - @PrevMonth08
			end
			else
			begin
				set @Month08 = @PrevMonth08 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 9 --
			if (@DebitSum > @PrevMonth09)
			begin
				set @Month09 = 0
				set @DebitSum = @DebitSum - @PrevMonth09
			end
			else
			begin
				set @Month09 = @PrevMonth09 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 10 --
			if (@DebitSum > @PrevMonth10)
			begin
				set @Month10 = 0
				set @DebitSum = @DebitSum - @PrevMonth10
			end
			else
			begin
				set @Month10 = @PrevMonth10 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 11 --
			if (@DebitSum > @PrevMonth11)
			begin
				set @Month11 = 0
				set @DebitSum = @DebitSum - @PrevMonth11
			end
			else
			begin
				set @Month11 = @PrevMonth11 - @DebitSum
				set @DebitSum = 0
			end

			-- remain of month 12 --
			if (@DebitSum > @PrevMonth12)
			begin
				set @Month12 = 0
				set @DebitSum = @DebitSum - @PrevMonth12
			end
			else
			begin
				set @Month12 = @PrevMonth12 - @DebitSum
				set @DebitSum = 0
			end
			...
			begin
				...
				set @DebitSum = @DebitSum - @CreditSum;
			end
			else
			begin
				if @MonthCode = '01' set @Month01 = @CreditSum - @DebitSum;
				if @MonthCode = '02' set @Month02 = @CreditSum - @DebitSum;
				if @MonthCode = '03' set @Month03 = @CreditSum - @DebitSum;
				if @MonthCode = '04' set @Month04 = @CreditSum - @DebitSum;
				if @MonthCode = '05' set @Month05 = @CreditSum - @DebitSum;
				if @MonthCode = '06' set @Month06 = @CreditSum - @DebitSum;
				if @MonthCode = '07' set @Month07 = @CreditSum - @DebitSum;
				if @MonthCode = '08' set @Month08 = @CreditSum - @DebitSum;
				if @MonthCode = '09' set @Month09 = @CreditSum - @DebitSum;
				if @MonthCode = '10' set @Month10 = @CreditSum - @DebitSum;
				if @MonthCode = '11' set @Month11 = @CreditSum - @DebitSum;
				if @MonthCode = '12' set @Month12 = @CreditSum - @DebitSum;
				set @DebitSum = 0;
			end
			...
			select @PrevCreditRemainFr = @CreditRemainFr, @PrevCreditRemainTo = @CreditRemainTo, @PrevMonthCode	= @MonthCode, @PrevDebitSum	= @DebitSum, @PrevCreditSum	= @CreditSum, @PrevMonth00 = @Month00, @PrevMonth01 = @Month01, @PrevMonth02 = @Month02, @PrevMonth03 = @Month03, @PrevMonth04 = @Month04, @PrevMonth05 = @Month05, @PrevMonth06 = @Month06, @PrevMonth07 = @Month07, @PrevMonth08 = @Month08, @PrevMonth09 = @Month09, @PrevMonth10 = @Month10, @PrevMonth11 = @Month11, @PrevMonth12 = @Month12
	...
	set @StrSelect = '
		...
		from #tblResult R
				...
				OUTER APPLY acc.funGetCodeInfo(R.AcntCode) AS F 
		where ' + @whr +@StrFrom+ '
		order by ' + @SortBy
