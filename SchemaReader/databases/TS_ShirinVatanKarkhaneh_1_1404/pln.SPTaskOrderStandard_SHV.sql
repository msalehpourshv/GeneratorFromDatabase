...
-- =============================================================================================
-- =============================================================================================
CREATE PROCEDURE pln.SPTaskOrderStandard_SHV
     @CallType    Int, 
...
------------------------------------------------------------------------------------------	
	...
	set @StrSelect = '
		...
		select pub.GetGoodsName (a.ProductID,1) GoodsName, a.DocDate TaskDate, c.ShiftTypeID, emp.funGetShiftTypeName(c.ShiftTypeID,1) ShiftTypeName, s.ProductionLineID
			...
			,isnull((select Max(PersonCount)
		from '+@StrSelect3+' bb where '+@StrWhere2+' and  a.ProcessID=bb.ProcessID And a.ProcessNo=bb.ProcessNo And a.FiscalYear=bb.FiscalYear And a.SerialNo=bb.SerialNo),0) OperatorCount'
  ...
  set @Colname2=''
	...
	set @StrSelect=' 
	SerialNo ,BaseSerialNo ,DocDate,ProductID,	GoodsName, TaskDate	,ShiftTypeName	,ProductionLineName	,PureWeight, ExitCountStandard, ExitCount, OrderCount, ProductCountStandard,ProductCount	
	...
	begin
		...
		set @StrSelect='SELECT  a.*, G.UnitID, UnitName from (SELECT  '+@StrSelect+','+@Colname2+' 	FROM 
						(
	...
	begin
		set @StrSelect='Select a.*, G.UnitID, UnitName from (SELECT  '+@StrSelect+',11 AA	FROM 
						(
							SELECT t.*, c.DocDate ,LoseGroupID, GoodsQuantity --,			
							FROM    #tblTempStansard3 t
							inner join pln.tblProduceOrderHdr c on t.BaseProcessID=c.ProcessID And t.BaseProcessNo=c.ProcessNo And t.BaseFiscalYear=c.FiscalYear And t.BaseSerialNo=c.SerialNo 
