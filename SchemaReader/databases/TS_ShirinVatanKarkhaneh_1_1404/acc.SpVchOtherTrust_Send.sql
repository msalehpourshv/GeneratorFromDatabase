...
-- Description   : 
-- =============================================
Create PROCEDURE [acc].[SpVchOtherTrust_Send]
	@intVchNo				Int,
	@intDocStep				TinyInt,
    @strVchDate				Char(10),
...
BEGIN
	...
	SELECT	@StoreID=StoreID,@AcntCode=AcntCode,@Discount=Discount,@Discount2=Discount2+Discount3,@TotalLineDiscount=TotalLineDiscount,
            ...
            @EarnestMoneyAcntCode=EarnestMoneyAcntCode,@TaxOverWorthCost=TaxOverWorthCost,
    ...
    SELECT	@PackingAcntCode=PackingAcntCode,@VisitorCostAcntCode=VisitorCostAcntCode, 
			@TaxAcntCode=TaxAcntCode,@SaleTaxOverWorthAcntCode=SaleTaxOverWorthAcntCode,@OtherTrustAcntCode=OtherTrustAcntCode
    FROM inv.tblStores 
	...
	SET @strRecDesc=' ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
	...
	BEGIN
		...
		While (@@Fetch_Status = 0)
			BEGIN
				IF @GoodsPrice <> 0
					BEGIN
						SET @strRecDesc2 = ' ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial + ' - ' + @GoodsName + ' - ' + ltrim(rtrim(str(@GoodsQuantity))) + @GoodsUnit + ' - فی ' + ltrim(rtrim(str(@GoodsPrice)))
	...
	END
		...
		IF @DiscountAcntCode is not null AND (@Discount + @Discount2 + @TotalLineDiscount) <> 0
			BEGIN
				SET @strRecDesc2 = ' تخفیف ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
				SET @intMaxDocRowNo = @intMaxDocRowNo + 1
				SET @intMaxRowNo = @intMaxRowNo + 1
	...
	INSERT INTO acc.tblVoucherDtl
			(SerialNo,SourceProcessID,SourceProcessNo,SourceFiscalYear,SourceSerialNo,DocDate,IsAutoDoc,SessionNo,VchKind,RowNo,DocRowNo,
				AcntCode,Debit,Credit,RecDesc,RecDesc2,SourceDocType) 
	VALUES	(@intVchNo,@intSourceProcessID,@intSourceProcessNo,@intSourceFiscalYear,@intSourceSerialNo,@strVchDate ,1,@SessionNo ,1,@intMaxRowNo ,@intMaxDocRowNo,
				@OtherTrustAcntCode,0,ROUND(@SumGoodsPrice,0) ,TS.pub.funChangeFarsiStrings(pub.funReverseForCrystal(@strRecDesc)),TS.pub.funChangeFarsiStrings(pub.funReverseForCrystal(@DescHdr)),0)
	...
	IF @TransportationCostAcntCode <> '' AND @TransportationCost <> 0
		BEGIN
			...
			SET @strRecDescTransCost = ' هزینه حمل ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
	...
	IF @TransportationIncomeAcntCode <> '' AND @TransportationIncome <> 0
		BEGIN
			...
			SET @strRecDescTrans = ' درآمد حمل ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
		...
		IF @PackingAcntCode	<> '' AND @PackingCost <> 0
			BEGIN
				...
				SET @strRecDescPacking =' هزینه بسته بندی ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
		...
		IF @TaxAcntCode	<> '' AND @TaxCost <> 0
			BEGIN
				...
				SET @strRecDescTax = ' مالیات تجمیع عوارض ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
		...
		IF @VisitorAcntCode	<> '' AND @VisitorCostAcntCode	<> '' AND @VisitorCost <> 0
			BEGIN
				...
				SET @strRecDescVisitor = ' هزینه بازاریاب ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
		...
		IF @EarnestMoneyAcntCode <> '' AND @EarnestMoney <> 0
			BEGIN
				...
				SET @strRecDescEarnestMoney = ' پیش دریافت ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
		...
		IF @SaleTaxOverWorthAcntCode <> '' AND @TaxOverWorthCost <> 0
			BEGIN
				...
				SET @strRecDescTaxOverWorth = ' مالیات بر ارزش افزوده ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
	...
	IF @OtherCostAcntCode <> '' AND @OtherCost <> 0
		BEGIN
			...
			SET @strRecDescOtherCost = 'سایر هزینه ها در ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
	...
	IF @OtherIncomeAcntCode <> '' AND @OtherIncome <> 0
		BEGIN
			...
			SET @strRecDescIncome = 'سایر درآمد ها در ارسال كالاهاي اماني دیگران نزد ما ' + @strFiscalSerial
