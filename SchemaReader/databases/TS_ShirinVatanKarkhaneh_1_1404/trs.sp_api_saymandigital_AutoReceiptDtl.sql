...
-- Description   : Alian
-- =============================================
create PROCEDURE [trs].[sp_api_saymandigital_AutoReceiptDtl]
...
begin
	...
	where ProcessID=1 and ProcessNo=@ProcessNo and SerialNo=@SerialNo and FiscalYear=@FiscalYear
	...
	INSERT INTO trs.tblPayDtl
		(ProcessID,ProcessNo,FiscalYear,SerialNo,RowNo,DocRowNo,DocDate,ChequeDate,PayTypeID,DebitCode,
		 Amount,ChequeNo,LocationID,BankTypeID,BranchCode,BranchName,AccOwnerName,RowDesc)
	values
		(1,@ProcessNo,@FiscalYear,@SerialNo,@RowNo,@RowNo,@DocDate,@DocDate,@PayTypeId,@BankCode,
