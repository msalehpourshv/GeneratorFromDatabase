...
-- =============================================
Create PROCEDURE inv.sp_api_saymandigital_AutoSaleHdr
@ProcessNo as Int,
@FiscalYear as Int,
@DocStep as tinyint,
@DocDate as CHAR(10),
...
BEGIN
 ...
 select @SALProcessNo= ProcessNo from sal.tblSaleOrderHdr where TransferSerialNo=@OrderId
 
 IF(@SALProcessNo=@ProcessNo)
 BEGIN
	
	if (select count(*) from inv.tblStorageDocsHdr where TransferSerialNo=@OrderId and ProcessID=90)=0
	BEGIN
	...
	IF(@StoreID <> '10500001' AND @StoreID<>'10500090')
	BEGIN
		
		SELECT @StoreLayer =Layer2 FROM 
		pub.tblCodeLayer WHERE TableName='inv.tblStores'
		
		SELECT @StoreID= RIGHT('00000'+@StoreID,@StoreLayer);
		SET @StoreID='60'+@StoreID
	END
	...
	SELECT @maxSerialNo =SerialNo from sal.tblSaleOrderHdr where TransferSerialNo=@OrderId and ProcessID=180
	...
	INSERT INTO inv.tblStorageDocsHdr
		...
		TransportationIncomeAcntCode,TransportationIncome,TransferSerialNo,VchDate)
	...
	SELECT 90, @ProcessNo, @FiscalYear, @maxSerialNo, @DocStep, @DocDate, @StoreID, @AcntCode, 
		...
		@TransportationIncomeAcntCode,@TransportationIncome,@TransferSerialNo,@DocDate
	
	select @BaseSerialNo =isnull(SerialNo,0) from sal.tblSaleOrderHdr where TransferSerialNo=@OrderId
	
	if @BaseSerialNo is not null or @BaseSerialNo<>''
	begin
 ...
 ELSE
 BEGIN 
  Set @StrErrorMessage = N'Nco سفارش فروش با فروش آن یکسان نیست'
  raiserror (@StrErrorMessage, 16, 1)
 End
...
Begin
	...
	Set @StrErrorMessage = N'سفارش فروش ثبت نشده است'
	raiserror (@StrErrorMessage, 16, 1)
END
