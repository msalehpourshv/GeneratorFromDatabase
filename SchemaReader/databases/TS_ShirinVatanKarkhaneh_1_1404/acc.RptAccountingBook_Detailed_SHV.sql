...
-- =============================================================================================
-- =============================================================================================
CREATE PROCEDURE acc.RptAccountingBook_Detailed_SHV
	@PartNumber		Int = 1,
...
DECLARE @StrWhere_SHV	NVarChar(max);
...
BEGIN
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND Substring(AcntCode, ' + LTrim(Str(@Part1Start)) + ', ' + LTrim(Str(Len(@AcntCode1Fr))) + ') >= ''' + @AcntCode1Fr + ''''
	End
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND Substring(AcntCode, ' + LTrim(Str(@Part1Start)) + ', ' + LTrim(Str(Len(@AcntCode1To))) + ') <= ''' + @AcntCode1To + ''''
	End
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND Substring(AcntCode, ' + LTrim(Str(@Part2Start)) + ', ' + LTrim(Str(Len(@AcntCode2Fr))) + ') >= ''' + @AcntCode2Fr + ''''
	End
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND Substring(AcntCode, ' + LTrim(Str(@Part2Start)) + ', ' + LTrim(Str(Len(@AcntCode2To))) + ') <= ''' + @AcntCode2To + ''''
	End
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND Substring(AcntCode, ' + LTrim(Str(@Part3Start)) + ', ' + LTrim(Str(Len(@AcntCode3Fr))) + ') >= ''' + @AcntCode3Fr + ''''
	End
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND Substring(AcntCode, ' + LTrim(Str(@Part3Start)) + ', ' + LTrim(Str(Len(@AcntCode3To))) + ') <= ''' + @AcntCode3To + ''''
	End
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND Substring(AcntCode, ' + LTrim(Str(@Part4Start)) + ', ' + LTrim(Str(Len(@AcntCode4Fr))) + ') >= ''' + @AcntCode4Fr + ''''
	End
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND Substring(AcntCode, ' + LTrim(Str(@Part4Start)) + ', ' + LTrim(Str(Len(@AcntCode4To))) + ') <= ''' + @AcntCode4To + ''''
	End
	...
	Begin
		...
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND (DocDate >= ''' + @DocDateFr + ''')'
	End
	If (@DocDateTo Is Not Null)
	Begin
		SET @StrWhere = @StrWhere + ' AND (DocDate <= ''' + @DocDateTo + ''')'
		SET @StrWhere_SHV = @StrWhere_SHV + ' AND (DocDate <= ''' + @DocDateTo + ''')'
	End
	...
	Set @StrSelectR = N'
		SELECT	0 SerialNo, DocDate, Debit, Credit' + @StrMainFeilds + ', RecDesc1, RecDesc2, 
				...
				1 AS DocRowNo,1 as RowNo, Cast(0 AS Bit) AS IsShowDetail, --0 SessionNo, 
	...
	SET @StrSelect = N'
		...
		(	SELECT	SerialNo,DocDate,Debit,Credit ' + @StrMainFeilds + ',
					...
					DocRowNo,RowNo,IsShowDetail,--SessionNo, 
					Emphasize, Emphasize2, Emphasize3,VchKind,CurrencyTypeID
	...
	-- =============================================================================
	IF Object_ID ('dbo.tbl_Temp_Rpt_Acc') Is Not Null DROP TABLE dbo.tbl_Temp_Rpt_Acc
	...
	begin
		...
		Set @StrSelect = ' 
			select *   ,'''+@ExtraParmsSubReport+''' as ExtraParmsSubReport 
			from (
	...
	--	WHERE AcntCode = ''11401 3173001''
	--),
	--CalculatedData AS (
	--	SELECT SerialNo, DocRowNo, DocDate, AcntCode, Debit, Credit, RecDesc, RecDesc2, CreditSum.TotalCredit, 
	--		   CreditSum.TotalCredit - SUM(Debit) OVER (ORDER BY DocDate ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS RemainingCredit
	--	FROM acc.tblVoucherDtl
	--	CROSS JOIN CreditSum
	--	WHERE AcntCode = ''11401 3173001''
	--)
	--SELECT c.SerialNo SHV_SerialNo, c.DocRowNo SHV_DocRowNo, c.DocDate SHV_DocDate, 
	--       Substring(c.AcntCode, ' + LTrim(Str(@Part2Start)) + ', ' + LTrim(Str(Len('c.AcntCode'))) + ') SHV_AcntCode, c.Debit SHV_Debit, c.Credit SHV_Credit, 
	--       c.RecDesc SHV_RecDesc, c.RecDesc2 SHV_RecDesc2, c.TotalCredit SHV_TotalCredit, c.RemainingCredit SHV_RemainingCredit,
	--	   CASE
	--		   WHEN LAG(c.RemainingCredit, 1, 0) OVER (ORDER BY c.DocDate) < 0 THEN c.Debit
	--		   ELSE 
	--			   CASE 
	--				   WHEN c.RemainingCredit < 0 THEN ABS(c.RemainingCredit)
	--				   ELSE 0
	--			   END
	--	   END AS SHV_RemainingDebit, t.*
	--	   t.YourOtherColumns -- جایگزین کنید با نام ستون‌هایی که از tbl_Temp_Rpt_Acc نیاز دارید
	--FROM CalculatedData c
	--LEFT JOIN dbo.tbl_Temp_Rpt_Acc t ON c.SerialNo = t.SerialNo AND c.DocRowNo = t.DocRowNo AND c.DocDate = t.DocDate And 
	--                                    Substring(c.AcntCode, ' + LTrim(Str(@Part2Start)) + ', ' + LTrim(Str(Len('c.AcntCode'))) + ') = t.AcntCode
	--ORDER BY c.DocDate;
	--'
	...
	SELECT c.SerialNo SHV_SerialNo, c.DocRowNo SHV_DocRowNo, c.DocDate SHV_DocDate, c.AcntCode SHV_AcntCode, c.Debit SHV_Debit, c.Credit SHV_Credit, 
	       c.RecDesc SHV_RecDesc, c.RecDesc2 SHV_RecDesc2, c.TotalCredit SHV_TotalCredit, c.RemainingCredit SHV_RemainingCredit,
		   CASE
			   WHEN LAG(c.RemainingCredit, 1, 0) OVER (ORDER BY c.DocDate) < 0 THEN c.Debit
			   ELSE 
