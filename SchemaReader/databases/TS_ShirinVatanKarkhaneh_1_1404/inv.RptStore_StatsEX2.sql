...
-- ----------------------------------------------
-- ==============================================
CREATE PROCEDURE [inv].[RptStore_StatsEX2]
	@ProcessID		Int = 90,
	@ProcessNo		Int = 1,
	@FiscalFr		Int = Null,
	@SerialFr		Int = Null,
...
Begin --============== S T A R T  C O D E ===================================================
	...
	if (@Count = 6)
		set @StrGroup = '(Count(*) > 5)'
	else
		set @StrGroup = '(Count(*) = ' + ltrim(str(@Count)) + ')'
	...
	select X.*, AH.VisitPathID1, AH.VisitPathID2, AH.VisitPathID3, AH.VisitPathID4,
			...
			(AD.Address1 + '' '' + AD.Address2) AcntAddr
	from
	(
		select T.AcntCode, sum(T.Price) SumPrice, Count(*) OrderCount
		from
		(
			select ' + @StrAcnt + ' AcntCode, 
					(
			...
			from inv.vwStorageDocsHdr H
				inner join acc.tblAcnt AH on AH.AcntCode = ' + @StrAcnt + ' and AH.PartNumber = ' + str(@PartNo) + '
				inner join acc.tblAcntDtl AD on AD.AcntCode = AH.AcntCode and AD.PartNumber = AH.PartNumber
			where ' + @StrWhere + '
		) T
		group by AcntCode
		having ' + @StrGroup + '
	) X	left join acc.tblAcnt    AH on AH.AcntCode = X.AcntCode and AH.PartNumber = ' + str(@PartNo) + '
		left join acc.tblAcntDtl AD on AD.AcntCode = X.AcntCode and AD.PartNumber = ' + str(@PartNo) + '
