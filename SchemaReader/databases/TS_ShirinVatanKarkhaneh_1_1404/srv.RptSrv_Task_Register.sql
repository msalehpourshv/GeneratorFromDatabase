...
-- =============================================
Create PROCEDURE [srv].[RptSrv_Task_Register] 
	@FiscalYear			INT=94,
...
-- ================ SELECT ===========================
	...
	SET @StrSelect2 = '
	SELECT DISTINCT 0 as ID, '''' As OperatorName, h.DocDate, d.*, a.*, a2.*, r.CompanyID,r.PhonerName, p.PersonnelID,
					 '''' As GoodsID, '''' As GoodsName, '''' As UnitID, '''' As UnitName,
	 ...
	 FROM srv.tblServiceTaskDtl  d
		...
		LEFT JOIN srv.tblServiceTaskHdr h
		ON d.ProcessID=h.ProcessID AND d.ProcessNo=h.ProcessNo
		AND d.FiscalYear=h.FiscalYear AND d.SerialNo=h.SerialNo
	
		LEFT JOIN srv.tblServiceTaskPersonnelDtl p
		ON p.ProcessID = d.ProcessID AND p.ProcessNo = d.ProcessNo
		AND p.FiscalYear = d.FiscalYear AND p.SerialNo = d.SerialNo  
		
		LEFT JOIN srv.tblServiceTaskAtm2 a
		ON a.ProcessID = d.ProcessID AND a.ProcessNo = d.ProcessNo
		AND a.FiscalYear = d.FiscalYear AND a.SerialNo = d.SerialNo 
		AND a.DocRowNo = d.DocRowNo 
		
		LEFT JOIN srv.tblServiceTaskAtm1 a2
		ON a2.ProcessID = d.ProcessID AND a2.ProcessNo = d.ProcessNo
		AND a2.FiscalYear = d.FiscalYear AND a2.SerialNo = d.SerialNo
		AND a2.DocRowNo = d.DocRowNo
		
		LEFT JOIN srv.tblServiceRequestHdr r
		ON r.ProcessID = a2.ProcessID AND r.ProcessNo = a2.ProcessNo
		AND r.FiscalYear = a2.RequestFiscalYear AND r.SerialNo = a2.RequestCode
		
		
		LEFT JOIN srv.tblServiceRequestDtl rd
		on rd.ProcessID = r.ProcessID AND rd.ProcessNo = r.ProcessNo
		AND rd.FiscalYear = r.FiscalYear AND rd.SerialNo = r.SerialNo
		...
		0 RowNo,0 DocRowNo,'''' TaskDtlDesc,1 Priority,'''' AgreeDate,'''' AgreeTime,
		...
		1 DocAtomRowNo,t.DocDate EventDate,'''' ExitTime,0 EventStatus,t.Duration Duration,
		t.StartTime StartTime,t.FinishTime EndTime,'''' EnterTime,t.ResultDesc PerformDesc,
		t.ServiceID ServicID,1 Qty,'''' FaultID,t.PersonnelID PersonelID,
		'''' ChangeSet,'''' FaultDesc,0 ProcessID, cast(SUBSTRING(DocDate,1,4)AS INT)as FiscalYear, 0 SerialNo, 1 RowNo,
		1 DocRowNo,1 AtomRowNo,1 DocAtomRowNo, 0 RequestCode,0 RequestRow,0 RequestFiscalYear,t.CompanyID CompanyID,'''' As PhonerName,
		t.PersonnelID PersonnelID,		t.GoodsID, [pub].[funGetGoodsName](t.GoodsID,' + LTrim(RTrim(@LangID)) + ') GoodsName, S.UnitID, U.UnitName,
		acc.funGetAcntName(t.CompanyID,'+ LTRIM(STR(@AcntPart)) +','+ LTRIM(str(@LangID)) +')  AcntName,
		prs.funGetPersonnelName(t.PersonnelID,'+LTRIM(str(@LangID))+')  PersonelName,
		acc.funGetServiceNameFull(t.ServiceID,'+ LTRIM(str(@LangID)) +') ServiceName,'''' FaultName,
		0 IsAsap
	 ...
	 BEGIN
		SET @StrSelect2 = '
		 SELECT DISTINCT 0 as ID,h.DocDate,d.*,a.*,a2.*,r.CompanyID,r.PhonerName,p.PersonnelID,
						 '''' As GoodsID, '''' As GoodsName, '''' As UnitID, '''' As UnitName,
						 acc.funGetAcntName(r.CompanyID,'+ ltrim(STR(@AcntPart)) +','+ LTRIM(str(@LangID)) +')AS  AcntName,
						 prs.funGetPersonnelName(p.PersonnelID,1)as PersonelName,
						 acc.funGetServiceNameFull(a.ServiceID,'+ LTRIM(str(@LangID)) +')AS ServiceName,
		 ...
		 ON p.ProcessID = d.ProcessID AND p.ProcessNo = d.ProcessNo
		 AND p.FiscalYear = d.FiscalYear AND p.SerialNo = d.SerialNo 
		 INNER JOIN srv.tblServiceTaskAtm2 a
		 ON a.ProcessID = d.ProcessID AND a.ProcessNo = d.ProcessNo
		 AND a.FiscalYear = d.FiscalYear AND a.SerialNo = d.SerialNo 
		 AND a.DocRowNo = d.DocRowNo 
		 LEFT JOIN srv.tblServiceTaskAtm1 a2
	...
	SET @StrSelect = @StrSelect2 + @StrSelect3
	-- Exeute --------------------------
	PRINT @StrSelect;
