...
-- =============================================
Create PROCEDURE [trs].[RptPayReceiptDocs_Summary] 
	...
	@DescMask			NVarChar(100) = Null, -- قسمتی از شرح برگه
	@DescMaskDtl		NVarChar(100) = Null, -- قسمتی از شرح سطر
...
declare @DistSerialNo int
declare @SettlmentSerialNo int
declare @SelectPayTypeID int
declare @PayTypeIDS  VarChar(100)
DECLARE @PayTyPeID as int= 0
DECLARE @GroupByPayTyPeID as bit
DECLARE @ShowAmontSummary as bit
DECLARE @SortCode	NVarChar(100)
...
--	SET @LanguageID = pub.funGetCurrentLanguageID();
	...
	SET @PayTyPeID	=pub.funSplitString(@RepInfo, '@', 6);
	SET @GroupByPayTyPeID   =pub.funSplitString(@RepInfo, '@', 7);
	SET @ShowAmontSummary   =pub.funSplitString(@RepInfo, '@', 8);
	SET @SortCode			=pub.funSplitString(@RepInfo, '@', 9);	
	
	IF @SortCode = '' OR @SortCode IS NULL SET @SortCode = 'D.SerialNo'
	
	SET @DistSerialNo =cast(pub.funSplitString(@ExtraParams, '@', 1) as int);
	
	--cast(pub.funSplitString(@ExtraParams, '@', 2) as int);  For report
	...
	SET @SettlmentSerialNo =cast(pub.funSplitString(@ExtraParams, '@', 5) as int);

	SET @Sgn1		= pub.funSplitString(@ExtraParams, '@', 6);
	SET @Sgn2		= pub.funSplitString(@ExtraParams, '@', 7);
	SET @Sgn3		= pub.funSplitString(@ExtraParams, '@', 8);
	SET @Sgn4		= pub.funSplitString(@ExtraParams, '@', 9);
	SET @Sgn5		= pub.funSplitString(@ExtraParams, '@', 10);	
	SET @HasSgn1	= pub.funSplitString(@ExtraParams, '@', 11);
	SET @HasSgn2	= pub.funSplitString(@ExtraParams, '@', 12);
	SET @HasSgn3	= pub.funSplitString(@ExtraParams, '@', 13);
	SET @HasSgn4	= pub.funSplitString(@ExtraParams, '@', 14);
	SET @HasSgn5	= pub.funSplitString(@ExtraParams, '@', 15);
	SET @NotSgn1	= pub.funSplitString(@ExtraParams, '@', 16);
	SET @NotSgn2	= pub.funSplitString(@ExtraParams, '@', 17);
	SET @NotSgn3	= pub.funSplitString(@ExtraParams, '@', 18);
	SET @NotSgn4	= pub.funSplitString(@ExtraParams, '@', 19);
	SET @NotSgn5	= pub.funSplitString(@ExtraParams, '@', 20);
	
	SET @year = RIGHT(db_name(),4)
	--------------------------------------------------------------------------------------------
 
	-- S E L E C T -----------------------------------------------------------------------------
if @ShowAmontSummary=0
	BEGIN
	Set @StrSelect = '
	SELECT	D.ProcessID, D.ProcessNo, D.FiscalYear, D.SerialNo, D.DocDate, D.DebitCode, D.CreditCode, H.VchNo, H.DescHdr, D.Amount, 
			CASE WHEN (D.ProcessID = 1 OR D.ProcessID = 40) THEN pub.GetBankName(D.DebitCode, ' + @LangID + ') ELSE pub.GetCodeName(D.DebitCode, ' + @LangID + ')  END AS DebtorNameHdr,
			CASE WHEN (D.ProcessID = 2 OR D.ProcessID = 40) THEN pub.GetBankName(D.CreditCode,' + @LangID + ') ELSE pub.GetCodeName(D.CreditCode, ' + @LangID + ') END AS CreditorNameHdr,
			( select sum(Amount*DateDuration)/sum(Amount)
				from (
					SELECT	 ChequeDate,Amount,	case When len(ChequeDate) <> 10 then 1 else  pub.funFarsiDateDiff(''Day'', DocDate , ChequeDate)  end  AS DateDuration
					FROM	trs.tblPayDtl
			...
			, [pub].[GetUserName](H.SessionNo1) AS UserName, D.PayTypeID,D.ChequeDate,D.ChequeNo,D.ChequeNoNew,D.NationalIDNumber
	, D.DocRowNo,(select Count(*) from trs.tblPayAtm AS A where  D.ProcessID = A.ProcessID AND D.ProcessNo = A.ProcessNo AND D.FiscalYear = A.FiscalYear AND D.SerialNo = A.SerialNo AND D.DocRowNo = A.DocRowNo) AtomCount
	FROM	trs.tblPayHdr AS H 
				INNER JOIN trs.tblPayDtl AS D ON  D.ProcessID = H.ProcessID AND D.ProcessNo = H.ProcessNo AND D.FiscalYear = H.FiscalYear AND D.SerialNo = H.SerialNo
	WHERE	H.FiscalYear = ' + @year + ' and ( ' + LTrim(Str(@ProcessNo))+' = 0  or D.ProcessNo = ' + LTrim(Str(@ProcessNo)) +')'
END
ELSE
BEGIN
 	Set @StrSelect = '
	SELECT	0 ProcessID, 0 ProcessNo, 0 FiscalYear, 0 SerialNo, 0 DocDate, 0 DebitCode, 0 CreditCode, 0 VchNo, 0 DescHdr, SUM(D.Amount)Amount, 
			'''' DebtorNameHdr,
			'''' CreditorNameHdr,
			0 DateDuration
			, '''' UserName, D.PayTypeID
...
End
	...
	If	(@ProcessID > 0)
		SET @StrSelect = @StrSelect + ' AND D.ProcessID = ' + LTrim(Str(@ProcessID))
	Else
		SET @StrSelect = @StrSelect + ' AND D.ProcessID < 3'
	...
	If (@DescMaskDtl Is Not Null) And (@DescMaskDtl <> '')
		SET @StrSelect = @StrSelect + ' AND (Replace(D.RowDesc, '' '', '''') LIKE N''%' + RTrim(Replace(@DescMaskDtl, ' ', '')) + '%'')'
	...
	If (@BehalfID = '0')
		...
		if @PayTyPeID != 0
		SET @StrSelect = @StrSelect + '  And   D.PayTypeID =' +Str(@PayTyPeID)
	
	if (@DistSerialNo is not null) and (@DistSerialNo<>'') 
		SET @StrSelect = @StrSelect + ' AND 
					(SELECT BaseDistributionSerialNo  
						FROM inv.tblStorageDocsHdr hd
	   						where hd.ProcessID=H.BaseProcessID 
	   						and hd.ProcessNo=H.BaseProcessNo
	   						and hd.FiscalYear=H.BaseFiscalYear
	   						and hd.SerialNo=H.BaseSerialNo)=' + ltrim(str(@DistSerialNo))
	if (@SettlmentSerialNo is not null) and (@SettlmentSerialNo<>'') 
		SET @StrSelect = @StrSelect + ' AND 
					(SELECT top 1 SerialNo  
						FROM  trs.tblSettlementDtl hd
	   						where hd.BasePayProcessID=H.ProcessID 
	   						and hd.BasePayProcessNo=H.ProcessNo
	   						and hd.BasePayFiscalYear=H.FiscalYear
	   						and hd.BasePaySerialNo=H.SerialNo)=' + ltrim(str(@SettlmentSerialNo))
...
-- Sort ------------------------------------------------------------------------------------
 IF @ShowAmontSummary=0
	SET @StrSelect = @StrSelect + ' ORDER BY ' + @SortCode 
 ELSE
	SET @StrSelect = @StrSelect + ' GROUP BY D.PayTypeID'
