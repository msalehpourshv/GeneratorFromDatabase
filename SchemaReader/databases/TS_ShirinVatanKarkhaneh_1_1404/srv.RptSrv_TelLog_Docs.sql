...
-- Create date   : 1391/05/17
-- Viewed By	 : 
-- Last Modified : 
-- Last Modifier : 
-- Description	 : گزارش تماسهای یک مشتری
-- =============================================
CREATE PROCEDURE [srv].[RptSrv_TelLog_Docs] 
...
--
	set @StrWhere = '(1=1) AND Duration <> ''00:00:00'' AND Duration <> '''' '
	
	IF (@CompanyID IS NOT null)
		SET @StrWhere = @StrWhere + ' AND CompanyID = ''' + @CompanyID + ''''
	...
	IF (@LineNumber IS NOT null)
		SET @StrWhere = @StrWhere + ' AND LineNumber = ''' + @LineNumber + ''''
...
-- ================ SELECT ===========================
	...
	DurationBySec,AcntName,CONVERT(int ,SUBSTRING(t.Duration,1,2),1) AS h,CONVERT(int ,SUBSTRING(t.Duration,4,2),1) AS  m,
	CONVERT(int ,SUBSTRING(t.Duration,7,2),1)  AS s
