...
create PROCEDURE  [sal].[Spsal_GoodsPriceForCustomerKindWithDateDtl]
	@UserID as int=0,
...
where H.FromDate<=@DocDate AND (H.ToDate='' OR H.ToDate>=@DocDate) AND H.CurrencyTypeID = '' and  NotShowInTablet = 'False' AND 1=1 and
...
begin
	...
	inner join inv.tblGoods G
	on D.GoodsID=G.GoodsID
	where   H.FromDate>=@DocDate AND H.ToDate<=@DocDate AND H.CurrencyTypeID = '' and NotShowInTablet = 'False' 
