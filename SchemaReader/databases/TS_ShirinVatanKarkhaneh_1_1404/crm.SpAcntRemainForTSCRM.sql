...
-- Last Modifier : 
-- ----------------------------------------------
-- Description	 : < مانده حساب مشتریان برای CRM  >
-- ==============================================
Create PROCEDURE crm.SpAcntRemainForTSCRM
	@AcntCode			Varchar(20), 
	@ToDate				char(10)
WITH ENCRYPTION
AS
...
select @Layer1Lan=Layer1 from pub.tblCodeLayer   where TableName='acc.tblAcnt' and PartNumber=1
	...
	insert into #tblRemain
		...
		WHERE AcntType NOT IN (91,92) AND VchKind <> 0  
		AND SUBSTRING(a.AcntCode,'+str(@PartStart)+' ,'+str(@PartLen)+' ) = '''+@AcntCode+ ''''
...
select  abs(Debit-Credit)	Remain 
		,case when Debit<Credit then  'بس'  else  'بد'end  RemainType
		, @AcntCode AcntCode
  from #tblRemain			
