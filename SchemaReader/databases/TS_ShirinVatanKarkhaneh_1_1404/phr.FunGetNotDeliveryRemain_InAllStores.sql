...
GO
CREATE FUNCTION [phr].[FunGetNotDeliveryRemain_InAllStores]
(
	@GoodsID VarChar(20), 
	@DocDate Char(10) ,
	@ExpDate CHAR(10),
...
Begin -- ====================================================
	...
	BEGIN
		...
		AND RH.DocDate <=@DocDate AND  RH.Deleted =0 
		 AND  (SELECT COUNT(*) from inv.tblStorageDocsDtl 
	            where BaseProcessID=RH.ProcessID And 
	            BaseSerialNo = RH.SerialNo And  
				BaseFiscalYear=RH.FiscalYear )=0 and IsSimilar=1
	...
	BEGIN
		...
		(CD.DeliveryToCustomer IS NULL OR CD.DeliveryToCustomer=0 )
		 AND pub.funFarsiDateAddDays('Day',RH.DocDate,@Day) >= @DateNow
		AND RH.DocDate <=@DocDate AND RD.ExpDate=@ExpDate AND  RH.Deleted =0
		AND  (SELECT COUNT(*) from inv.tblStorageDocsDtl 
	            where BaseProcessID=RH.ProcessID And 
	            BaseSerialNo = RH.SerialNo And  
				BaseFiscalYear=RH.FiscalYear )=0 and IsSimilar=0
