USE TS_ShirinVatanKarkhaneh_1_1404
GO
SET ANSI_NULLS, QUOTED_IDENTIFIER ON
GO
-- =========== TS-QC:NOTOK ========================
-- Author        : Alian Pour
-- Create date   : 1400/09/01
-- Viewed By	 : 
-- Last Modified : 
-- Description   : 
-- =============================================
Create PROCEDURE [inv].[sp_api_TakroSystem_CreateOrUpdateStoreRequestsDtl]
@ProcessNo as tinyint,
@FiscalYear as SMALLINT,
@SerialNo as Int,
@DocStep as tinyint,
@GoodsID AS VARCHAR(20),
@Quantity as FLOAT,
@SubUnitID as varchar(30),
@AcntCode AS VARCHAR(20),
@DescDtl as NVARCHAR(500),
@DocDate as CHAR(10),
@StoreID AS VARCHAR(20)


WITH ENCRYPTION
 AS
BEGIN

Declare @StrErrorMessage As Nvarchar(1024)
Declare @MaxRowNo as Int
Declare @StoreId2  As Nvarchar(50)
BEGIN TRY

	

	IF (SELECT Count(*) FROM inv.tblGoods
		where GoodsID=@GoodsID )<>1
	BEGIN
		Set @StrErrorMessage = N'کد کالا صحیح نیست'
		raiserror (@StrErrorMessage, 16, 1)
	END	

	
	SELECT @StoreId2=StoreID2 from inv.tblStoresRequestsHdr
	where ProcessID=127 and ProcessNo=@ProcessNo and  FiscalYear=@FiscalYear and SerialNo=@SerialNo 

	--Update or quantity=0 =>Delete
	if (select count (*) from inv.tblStoresRequestsDtl
	 where GoodsID=@GoodsID and SerialNo=@SerialNo and 
		   ProcessNo=@ProcessNo and ProcessID=127 and FiscalYear=@FiscalYear)>0
	BEGIN
		SELECT @MaxRowNo = RowNo
		FROM inv.tblStoresRequestsDtl
		WHERE ProcessID=127
		 AND ProcessNo=@ProcessNo
		 AND FiscalYear=@FiscalYear
		 AND SerialNo=@SerialNo
		 AND GoodsID=@GoodsID

		if @Quantity=0

		--Delete
		BEGIN
			print 'delete'
			DELETE FROM  inv.tblStoresRequestsDtl
			where RowNo=@MaxRowNo AND 
				  ProcessID=127 AND 
				  ProcessNo=@ProcessNo AND 
				  FiscalYear=@FiscalYear AND 
				  SerialNo=@SerialNo AND 
				  GoodsID=@GoodsID

			Select 'Delete' As ActionName,@MaxRowNo

		END

		ELSE

		--Update
		BEGIN
			print 'update'
			update inv.tblStoresRequestsDtl
			set  StoreID=@StoreID, AcntCode=@AcntCode,StoreID2=@StoreId2,
			GoodsID=@GoodsID, SubUnitQuantity=@Quantity,GoodsQuantity=@Quantity,DescDtl=@DescDtl
			where
			ProcessID=127 and ProcessNo=@ProcessNo and  FiscalYear=@FiscalYear and SerialNo=@SerialNo and RowNo=@MaxRowNo

			Select 'Update' As ActionName,@MaxRowNo
		END
	End

	--Create
	ELSE
	
	Begin
	
		SELECT @MaxRowNo = isnull(MAX(RowNo),0)
		FROM inv.tblStoresRequestsDtl
		WHERE ProcessID=127
		 AND ProcessNo=@ProcessNo
		 AND FiscalYear=@FiscalYear
		 AND SerialNo=@SerialNo
			 
		SET @MaxRowNo = @MaxRowNo +1

		INSERT INTO inv.tblStoresRequestsDtl
			(ProcessID, ProcessNo, FiscalYear, SerialNo, RowNo, DocRowNo,StoreID2,
			DocStep, DocDate, StoreID, AcntCode,
			GoodsID, SubUnitID, SubUnitQuantity,GoodsQuantity,
			DescDtl)
	
		select 127, @ProcessNo, @FiscalYear, @SerialNo, @MaxRowNo , @MaxRowNo ,@StoreId2,
			@DocStep, @DocDate, @StoreID, @AcntCode, 
		    @GoodsID,UnitID ,@Quantity ,@Quantity  ,
		    @DescDtl
		FROM inv.tblGoods 
		where GoodsID=@GoodsID

		Select 'Create' As ActionName,@MaxRowNo
	End
	
    
END TRY
BEGIN CATCH

	Set @StrErrorMessage = ERROR_MESSAGE() 
	raiserror (@StrErrorMessage, 16, 1)

END CATCH

END	

GO
