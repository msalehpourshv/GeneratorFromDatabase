...
-- Description   : گزارش سرجمع فروش و برگشتی به تفکیک مشتری
-- ==============================================
CREATE PROCEDURE [sal].[RptSale_Summary3]
...
Begin --============== S T A R T  C O D E ===================================================
	...
	SET @AcntPart	= Substring(@RepOptions, 2, 1) -- 1 is used 
	
	set @PartStart = 0
	set @PartLen = 0
	
	print @AcntPart	
	exec [acc].[SpAcc_PartInfo] @AcntPart, @PartStart out, @PartLen out
	...
	set @AcntCode = 'Substring(D.AcntCode, ' + ltrim(str(@PartStart)) + ', ' + ltrim(str(@PartLen)) + ')'

	SET @StrSelect = '
	select T.*, [pub].[funGetGoodsName](T.GoodsID,' + LTrim(RTrim(@LangID)) + ') GoodsName,
		   IsNull([inv].[FunGetGoodsBarCode] (T.GoodsID), '''') BarCode, A.AcntName, 0 PercentQ, 0 PercentP
	from
	(
		select	D.GoodsID, ' + @AcntCode + ' AcntCode,
				...
				SUM(CASE WHEN D.ProcessID = 90  THEN D.GoodsQuantity*D.GoodsPrice - D.DiscountDtl ELSE 0 END) AS PriceS,
				SUM(CASE WHEN D.ProcessID = 100 THEN D.GoodsQuantity*D.GoodsPrice - D.DiscountDtl ELSE 0 END) AS PriceR
		from    inv.tblStorageDocsDtl D 
		where   ' + @StrWhere + '
		group by ' + @AcntCode + ', D.GoodsID 
	) T 
	left join acc.tblAcntDtl  A on A.AcntCode = T.AcntCode and A.PartNumber = ' + ltrim(str(@AcntPart)) + '
	LEFT JOIN inv.tblGoodsDtl G ON G.GoodsID=SUBSTRING(T.GoodsID,' + ltrim(rtrim(STR(@str_Goods+1))) + ',' + ltrim(rtrim(STR(@str_GoodsSum))) + ') AND G.PartNumber=' + ltrim(rtrim(STR(@UnitPart)))+ '
	'
	------------------------------------------------------------
	-- Sort Clause ---------------------------------------------
