...
-- =============================================
Create PROCEDURE [acc].[RptAccountingBook_Summary]
	...
	@RecDesc		nVarChar(4000) = null,
	@RecDesc2		nVarChar(4000) = null,
...
DECLARE @StrDebitR		NVarChar(max);
DECLARE @StrCreditR		NVarChar(max);
...
BEGIN
	...
	if (@UseCurrency = 1) 
			set @StrMainFeilds = ' ,Sum(MainDebit) MainDebit, Sum(MainCredit) MainCredit'
		else
			set @StrMainFeilds = '  ,Sum(Debit) MainDebit, Sum(Credit) MainCredit'
...
--=========================
	...
	SET @StrSelectR = N'
		SELECT	SerialNo,OldSerialNo,DocDate,' + @StrDebitR + ' Debit,' + @StrCreditR + ' Credit ,MainDebit,  MainCredit,
				DocTypeCode,Group1,Group2,Group3,Group4
		FROM
		(
			SELECT	0 SerialNo,0 OldSerialNo,Cast(''-'' AS VarChar(10)) DocDate,
				Sum(Debit) Debit,Sum(Credit) Credit' + @StrMainFeilds + ',-1 AS DocTypeCode,
				' + @Group1 + ' As Group1,
				' + @Group2 + ' As Group2,
				' + @Group3 + ' As Group3,
				' + @Group4 + ' As Group4
			FROM ' + @StrFrom + '  D INNER JOIN acc.tblVoucherHdr H on D.SerialNo=H.SerialNo
			WHERE ' + @StrWhereR + '
			GROUP BY ' + @Group1 
				
	If @Group2Exist = 1
		SET @StrSelectR = @StrSelectR + ',' + @Group2
	If @Group3Exist = 1 
		SET @StrSelectR = @StrSelectR + ',' + @Group3
	If @Group4Exist = 1 
		SET @StrSelectR = @StrSelectR + ',' + @Group4

	SET @StrSelectR = @StrSelectR + '
		) R 
		WHERE (R.Debit Is Not Null) OR (R.Credit Is Not Null) '

	If (@GroupByDate = 1)
		SET @StrSerialNo = '0 AS SerialNo'
	Else
		SET @StrSerialNo = 'D.SerialNo'

	SET @StrSelect = '
		SELECT ' + @StrSerialNo + ',OldSerialNo,D.DocDate,Sum(Debit) Debit,Sum(Credit) Credit' + @StrMainFeilds + ',
				(Case When Credit=0 Then 0 Else 1 End) DocTypeCode,
				' + @Group1 + ' As Group1,
				' + @Group2 + ' As Group2,
				' + @Group3 + ' As Group3,
				' + @Group4 + ' As Group4
		FROM	' + @StrFrom + '  D INNER JOIN acc.tblVoucherHdr H on D.SerialNo=H.SerialNo ' + @StrAcntWhere + '
		WHERE ' + LTrim(RTrim(@StrWhere)) + '
		GROUP BY D.SerialNo,OldSerialNo,D.DocDate,(Case When Credit=0 Then 0 Else 1 End),' + @Group1 

	If @Group2Exist = 1 
		SET @StrSelect = @StrSelect + ',' + @Group2
	If @Group3Exist = 1 
		SET @StrSelect = @StrSelect + ',' + @Group3
	If @Group4Exist = 1 
		SET @StrSelect = @StrSelect + ',' + @Group4

	SET @StrSelect = N'
		SELECT SerialNo,OldSerialNo,DocDate,Sum(Debit) Debit,Sum(Credit) Credit' + @StrMainFeilds + ',
			DocTypeCode,Group1,Group2,Group3,Group4
		FROM
			( ' + @StrSelect + '
			) T 
		GROUP BY SerialNo,OldSerialNo,DocDate,DocTypeCode,Group1,Group2,Group3,Group4 '

	-- If include remain then attach remain & union it	
	If (@RemainMode <> 0)
		SET @StrSelect = 
		    LTrim(RTrim(@StrSelectR)) + ' 
			UNION ALL 
			' + LTrim(RTrim(@StrSelect)) 
		
	SET @StrSelect = N'
	SELECT	T.*,
			(select DocDesc2 from acc.tblVoucherHdr H where H.SerialNo=T.SerialNo) as DocDesc2,
	...
	If (@GroupByDate = 1)
		...
		ORDER BY DocDate,DocTypeCode,Group1,Group2,Group3,Group4'
	Else
	begin
		if (@SortBySerial = 1)
			SET @StrSelect = @StrSelect + ' 
			ORDER BY OldSerialNo,DocTypeCode,Group1,Group2,Group3,Group4 '
		else
			SET @StrSelect = @StrSelect + ' 
			ORDER BY SerialNo,DocTypeCode,Group1,Group2,Group3,Group4 '
	end
	Print @StrSelect;
