...
-- Viewed By	 : 
-- Last Modified : 1390/07/23
-- Last Modifier : TakroSystem\Zia
-- Description   : آمار میانگین انبار - طرف حسابها
-- ==============================================
Create PROCEDURE [inv].[RptStore_Average_Acnt] 
	@ProcessID			Int,
	@ProcessNo			Int = Null,
...
Begin --============== S T A R T  C O D E ===================================================
	...
	IF (@DecReturn = 1 ) -- کسر برگشتیهای فروش
		SET @StrWhere = '(D.' + @StrPrice + ' <> 0) AND (D.ProcessID IN (' + @StrPID + ', ' + @ReturnPID + '))'
	ELSE
...
---------------------------------------------------------------------------
	...
	SET @StrFrom = 'inv.tblStorageDocsDtl D
		...
		(
			...
			) B ON (D.SerialNo = B.LastSerialNo) AND (D.RowNo = B.LastRowNo) AND (D.GoodsID = B.GoodsID)
			WHERE ' + @StrWhere + '
	...
	IF (@ShowOverload = 1) 
	SET @StrSelect = '
	SELECT D.AcntCode, pub.GetCodeName(D.AcntCode, ' + @LangID + ') AS AcntName,

			(ROUND(sum(CASE WHEN D.ProcessID in (' + @StrPID + ') THEN (D.' + @StrPrice + ' * GoodsQuantity + AtomAmount ) ELSE 0 END), 0) -
			ROUND(sum(CASE WHEN D.ProcessID = ' + LTrim(Str(@ReturnPID)) + ' THEN (D.' + @StrPrice + ' * GoodsQuantity + AtomAmount  ) ELSE 0 END), 0)  
			)/(
			ROUND(sum(CASE WHEN D.ProcessID in (' + @StrPID + ') THEN (D.GoodsQuantity) ELSE 0 END), 0) -
			ROUND(sum(CASE WHEN D.ProcessID = ' + LTrim(Str(@ReturnPID)) + ' THEN (D.GoodsQuantity) ELSE 0 END), 0) )AS AvgPrice,

			ROUND(MIN(CASE WHEN D.ProcessID in (' + @StrPID + ') THEN ((D.' + @StrPrice + ' * GoodsQuantity + AtomAmount) / GoodsQuantity) ELSE 0 END), 0) -
			ROUND(MIN(CASE WHEN D.ProcessID = ' + LTrim(Str(@ReturnPID)) + ' THEN ((D.' + @StrPrice + ' * GoodsQuantity + AtomAmount) / GoodsQuantity) ELSE 0 END), 0) AS MinPrice,

			ROUND(MAX(CASE WHEN D.ProcessID in (' + @StrPID + ') THEN ((D.' + @StrPrice + ' * GoodsQuantity + AtomAmount) / GoodsQuantity) ELSE 0 END), 0) -
			ROUND(MAX(CASE WHEN D.ProcessID = ' + LTrim(Str(@ReturnPID)) + ' THEN ((D.' + @StrPrice + ' * GoodsQuantity + AtomAmount) / GoodsQuantity) ELSE 0 END), 0) AS MaxPrice,

		 COUNT(*) AS [Count], 0 AS LastPrice, ' + 
		 CASE WHEN (@SumVal > 0) THEN
			'ROUND(SUM((D.' + @StrPrice + ' * GoodsQuantity + AtomAmount) / GoodsQuantity), 0) / ' + LTRim(Str(@SumVal)) 
		 ELSE
			'0'
		 END
		 + ' AS Percentage'
	...
	SELECT	D.AcntCode, pub.GetCodeName(D.AcntCode, ' + @LangID + ') AS AcntName,
			...
			(ROUND(sum(CASE WHEN D.ProcessID in (' + @StrPID + ') THEN (D.' + @StrPrice + '* D.GoodsQuantity ) ELSE 0 END), 0) -
			ROUND(sum(CASE WHEN D.ProcessID = ' + LTrim(Str(@ReturnPID)) + ' THEN (D.' + @StrPrice + '* D.GoodsQuantity ) ELSE 0 END), 0) 
			)/(			
			ROUND(sum(CASE WHEN D.ProcessID in (' + @StrPID + ') THEN (D.GoodsQuantity) ELSE 0 END), 0) -
			ROUND(sum(CASE WHEN D.ProcessID = ' + LTrim(Str(@ReturnPID)) + ' THEN (D.GoodsQuantity) ELSE 0 END), 0) )AS AvgPrice,

			ROUND(MIN(CASE WHEN D.ProcessID in (' + @StrPID + ') THEN (D.' + @StrPrice + ') ELSE 0 END), 0) -
			ROUND(MIN(CASE WHEN D.ProcessID = ' + LTrim(Str(@ReturnPID)) + ' THEN (D.' + @StrPrice + ') ELSE 0 END), 0) AS MinPrice,

			ROUND(MAX(CASE WHEN D.ProcessID in (' + @StrPID + ') THEN (D.' + @StrPrice + ') ELSE 0 END), 0) -
			ROUND(MAX(CASE WHEN D.ProcessID = ' + LTrim(Str(@ReturnPID)) + ' THEN (D.' + @StrPrice + ') ELSE 0 END), 0) AS MaxPrice,
