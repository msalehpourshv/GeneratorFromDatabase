...
-- ==============================================
CREATE PROCEDURE [sal].[RptSalePay_Header]
...
Begin --============== S T A R T  C O D E ===================================================
	...
	SET @StrSelect = '
	SELECT	H.*, S1.StoreName, S2.StoreName StoreName2,
			...
			[pub].[funGetGoodsName](ProductID,' + LTrim(RTrim(@LangID)) + ') ProductName,
		    IsNull([inv].[FunGetGoodsBarCode] (ProductID), '''') BarCode,
			...
			TR.TransporterName, F.VisitPathID1, F.VisitPathID2, F.VisitPathID3, F.VisitPathID4, LD.LocationName, F.Tel, F.Address1 + '' '' + F.Address2 AcntAddress, F.Mobile,
			IsNull((
			...
			[Discount]+[Discount2]+[TotalLineDiscount] DiscountSum,
			[AfterSaleDiscount]+[OtherCost]+[TransportationCost]+[FixCost]+[VisitorCost]+[DistributeAmount] AS DecreaseSum,
			[OtherIncome]+[TransportationIncome]+[PackingCost]+[TaxCost]+[TaxOverWorthCost]+[TollOverWorthCost]+[IAToll] AS IncreaseSum
	FROM	inv.vwStorageDocsHdr H 
			INNER JOIN	inv.tblStoresDtl S1 ON H.StoreID  = S1.StoreID AND S1.LanguageID = ' + @LangID + '
