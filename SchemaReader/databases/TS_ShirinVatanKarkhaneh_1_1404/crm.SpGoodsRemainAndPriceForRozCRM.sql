...
-- ==============================================
--crm.SpGoodsRemainAndPriceForRozCRM '1520009','1402/04/04'
Create PROCEDURE crm.SpGoodsRemainAndPriceForRozCRM
	@GoodsID			Varchar(20), 
	@DocDate				char(10)
WITH ENCRYPTION
AS
BEGIN
	...
	if (@DocDate<> '' and  not (@DocDate is null))
			set @StrSelect= @StrSelect+ '	and D.DocDate<='''+@DocDate+''' '
	if (@GoodsID<> '' and  not (@GoodsID is null))
			set @StrSelect= @StrSelect+ '	and D.GoodsID='''+@GoodsID+''' '
    ...
    select *,
	[sal].[funGetSaleOrderGoodsRemain_Store](GoodsID,StoreID,@DocDate,RIGHT(DB_NAME(),4),0) SaleOrderGoodsRemain,
	[inv].[funGetGoodsRemainInPreSale](GoodsID,StoreID,@DocDate,'False',1,0,3) PreSaleGoodsRemain
	from #GoodsQTYRmain
