...
-- Description   : 
-- =============================================
CREATE FUNCTION [sal].[funGetSaleOrderGoodsRemainDocStep]
(	
	@GoodsID		Varchar(20),
	@StoreID		Varchar(20),
	@DocDate		Char(10),
...
BEGIN
	...
	IF @GetRemainSaleOrder = 'True'
		BEGIN
			...
			--	  (@UserPriceID =0 or UserPriceID=@UserPriceID) AND 
			--	  DocStep>1
	...
	ELSE
		BEGIN
			...
			(
				...
				WHERE ProcessID=180  
				EXCEPT
				SELECT BaseProcessID,BaseProcessNo,BaseFiscalYear,BaseSerialNo FROM inv.tblStorageDocsHdr
			...
			WHERE S.ProcessID = 180 AND
				  DocDate <= @DocDate AND 
				  S.FiscalYear= @FiscalYear AND (StoreID = '' OR @StoreID IS NULL OR (StoreID = @StoreID))  AND 
				  GoodsID = @GoodsID  AND (@UserPriceID =0 or UserPriceID=@UserPriceID)  AND 
				  DocStep>1
