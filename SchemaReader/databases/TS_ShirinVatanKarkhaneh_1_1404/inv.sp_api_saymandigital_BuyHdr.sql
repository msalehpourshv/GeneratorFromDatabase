...
-- Description   : 
-- =============================================
Create PROCEDURE [inv].[sp_api_saymandigital_BuyHdr]
@ProcessNo as Int,
@FiscalYear as Int,
@DocStep as tinyint,
@DocDate as CHAR(10),
...
BEGIN TRY
	...
	if (select count(*) from inv.tblStorageDocsHdr where TransferSerialNo=@TransferSerialNo and ProcessID=55)=0
	BEGIN
		...
		declare @debit as varchar(100)
		select @debit=substring( StockAcntCode ,7,14) from inv.tblStores
		...
		SELECT 55, @ProcessNo, @FiscalYear, @maxSerialNo, @DocStep, @DocDate, @StoreID,'91502 '+ @debit, 
		@Amount,@Price,@DiscountPercent,@Discount,@Discount2, @TotalLineDiscount,@DocDesc+' کد حساب:  '+@AcntCode,
		...
		SELECT 55 ProcessID ,@maxSerialNo SerialNo,@AcntCode AcntCode,@StoreID StoreID,@SaleTypeID SaleTypeID,0 IsExist,@BaseSerialNo BaseSerialNo
	End
	
	ELSE
		SELECT 55 ProcessID ,1 SerialNo,'' AcntCode,1 StoreID,'' SaleTypeID,1 IsExist,0 BaseSerialNo
