...
-- Description   : 
-- =============================================
Create PROCEDURE [acc].[SpVchReceipt2]
	@intVchNo				Int,
	@intDocStep				TinyInt,
    @strVchDate				Char(10),
...
DECLARE @trs_CurrencyTypeInRow BIT;
	...
	SELECT	RD.CurrencyRate,RD.CurrencyTypeID,PayTypeID,Amount,RowDesc,CreditCode,DebitCode,ChequeNo,ChequeDate,AccountNo,DocRowNo,CurrencyAmount,ID
	...
	Fetch NEXT From Cursor_PayDtl Into @CurrencyRateDtl,@CurrencyTypeIDDtl,@PayTypeID,@Amount,@RowDesc,@CreditCode,@DebitCode,@ChequeNo,@ChequeDate,@AccountNo,@DocRowNo,@CurrencyAmount,@BaseID
		...
		BEGIN
			...
			IF @Amount <> 0
				BEGIN
					...
					SET @strRecDesc = ' برگ دریافت ' + @strSourceProcessNo + ' شماره' + LTRIM(RTRIM(STR(@intSourceFiscalYear))) + '/' + LTRIM(RTRIM(STR(@intSourceSerialNo)))
						...
						BEGIN
							...
							INSERT INTO acc.tblVoucherDtl
									(SerialNo,SourceProcessID,SourceProcessNo,SourceFiscalYear,SourceSerialNo,SourceDocRowNo,DocDate,IsAutoDoc,SessionNo,VchKind,RowNo,DocRowNo,
										AcntCode,Debit,Credit,RecDesc,RecDesc2,SourceDocType,CurrencyAmount,CurrencyTypeID ,BaseID,VisitorAcntCode) 
							VALUES	(@intVchNo,@intSourceProcessID,@intSourceProcessNo,@intSourceFiscalYear,@intSourceSerialNo,@DocRowNo,@strVchDate ,1,@SessionNo ,@VchKind,@intMaxRowNo,@intMaxDocRowNo,
										@AtomAcntCode,0,@AtomAmount,TS.pub.funChangeFarsiStrings(pub.funReverseForCrystal(@AcntName)),@AtomDesc,0,@CurrencyAmountTmp,@CurrencyTypeIDTmp,@BaseID,@VisitorAcntCode)			
			...
			ELSE
				...
				BEGIN
					...
					IF @CreditCode=''
					BEGIN
							set @strMsgText='کد پرداخت کننده خالی می باشد'
