...
-- Description	 : 
-- ==============================================
CREATE PROCEDURE [pln].[RptPln_TaskFaults2_Stats_Sum]
...
Begin
	...
	IF (@SortFields		Is Null)	SET @SortFields = 'DepartmentID';
	...
	-- S E L E C T ------------------------------------------------------------
	SET @StrSelect = '
	select T.*,	DP.DepartmentName
	from
	(
		select  SD.DepartmentID,
				isnull(Sum(D.TotalTime),0) TotalTime, 
				isnull(sum(I.GoodsQuantity), 0) ProductCount,
				isnull(Sum(SD.LastProduceStepTime*(D.AcceptableCount+D.UnacceptableCount)),0) StandardTime
		from	pln.tblTaskOrderDtl D
					...
					inner join pln.tblProduceStepDtl SD ON (SD.ProductID = H.ProductID) AND (SD.SerialNo = D.ProduceStepSerialNo) and (SD.ProduceStepID = D.ProduceStepID)
  					inner join pln.tblProduceStepHdr SH ON (SH.ProductID = H.ProductID) AND (SH.SerialNo = D.ProduceStepSerialNo) 
		where ' + @StrWhere + '
		group by SD.DepartmentID
	) T 
	  left  join prs.tblDepartmentsDtl DP ON DP.DepartmentID = T.DepartmentID
	order by ' + @SortFields
   	---------------------------------------------------------------------------
	---------------------------------------------------------------------------
	PRINT @StrSelect;
